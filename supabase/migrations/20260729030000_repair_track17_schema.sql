...
-- Authoritative repair for the Aramex / 17TRACK schema.
-- The original 20260729010000 migration was marked applied after its transaction
-- had already rolled back, so this migration recreates every required object.
...
begin
  ...
  -- A row variable cannot be part of a multi-item INTO target list. Select the
  -- UUID and order JSON first, then load the shipment row separately.
  select s.id, to_jsonb(o)
    into v_shipment_id, v_order
  from public.international_shipments s
...
-- Force PostgREST to see the newly created tables immediately.
