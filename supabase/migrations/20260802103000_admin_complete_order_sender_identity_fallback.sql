...
-- Preserve a valid sender identity while changing the canonical merchant.
--
-- Some historical merchant records use a generic trade name such as
-- "Day Night Merchant". The complete editor previously copied that generic value
-- into orders.sender_name, and final validation correctly rejected the save.
-- This wrapper now resolves a valid sender identity without weakening validation:
-- explicit form value -> merchant owner -> valid trade name -> existing order value.
...
begin
  ...
  v_result := public.admin_update_order_complete_verified(v_effective_payload);
...
create or replace function public.admin_complete_order_sender_identity_health()
returns jsonb
language sql
security definer
stable
...
as $$
  select jsonb_build_object(
    'ok', to_regprocedure('public.admin_update_order_complete_verified_v2(jsonb)') is not null,
    'sender_name_order', jsonb_build_array(
      'explicit_form_value',
      'merchant_owner_name',
      'valid_trade_name',
      'existing_order_sender_name'
    ),
    'generic_sender_names_rejected', jsonb_build_array('day night merchant', 'unknown', 'n/a'),
    'final_validation_required', true,
    'checked_at', now()
  );
$$;

revoke all on function public.admin_complete_order_sender_identity_health() from public, anon;
grant execute on function public.admin_complete_order_sender_identity_health() to authenticated, service_role;
