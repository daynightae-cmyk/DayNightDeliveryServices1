...
-- Corrective migration for the Aramex / 17TRACK schema.
-- The original 20260729010000 migration was marked applied after its transaction
-- failed at a multi-item INTO statement, so none of its tables were created.
...
grant select, insert, update, delete on public.international_shipments to authenticated;
grant select, insert, update, delete on public.international_tracking_events to authenticated;
grant select on public.track17_webhook_logs, public.track17_api_logs, public.track17_quota_cache to authenticated;
grant all on public.international_shipments, public.international_tracking_events,
  public.track17_webhook_logs, public.track17_api_logs, public.track17_quota_cache to service_role;
...
comment on table public.international_shipments is
  'DAY NIGHT Aramex shipments registered with 17TRACK v2.4.';
comment on function public.daynight_public_international_tracking(text) is
  'Returns a limited public-safe international tracking payload.';
