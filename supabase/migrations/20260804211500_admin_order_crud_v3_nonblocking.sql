      ...
      if v_candidate_merchant is null then
        ...
        if v_merchant.id is null then
          ...
          if not (v_patch ? 'merchant_code') then
            v_patch := jsonb_set(v_patch, '{merchant_code}', to_jsonb(v_merchant.merchant_code), true);
          end if;
          if not (v_patch ? 'merchant_name') then
            v_patch := jsonb_set(v_patch, '{merchant_name}', to_jsonb(v_merchant.trade_name), true);
          end if;
          begin
...
set search_path = public, auth, extensions, pg_temp
as $$
  select public.admin_update_order_status_verified(
...
begin
  ...
  v_retention := v_before.financial_posted_at is not null
    or exists (select 1 from public.order_financial_settlements s where s.order_id=v_order_id::text)
    or exists (select 1 from public.financial_account_entries e where e.order_id=v_order_id::text)
    or exists (select 1 from public.cod_collections c where c.order_id=v_order_id);
