...
expect(pdfActions, /كشف راتب PDF|Payroll PDF/, "Employee card exposes a dedicated payroll PDF action");
expect(pdfActions, /مشاركة الكشف|Share statement/, "Employee card supports direct file sharing with download fallback");
expect(pdfActions, /fetchEmployeePayrollSnapshot/, "PDF uses authoritative payroll data for the selected period");
