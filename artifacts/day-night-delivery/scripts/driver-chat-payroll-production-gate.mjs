...
expect(driverContact,/prepareDeterministicDriverWhatsApp/,"Driver contact uses the deterministic action-specific message layer");
expect(deterministicDriverMessages,/prepareWhatsAppMessage/,"Deterministic messages preserve the centralized recipient and logging engine");
expect(driverContact,/openPreparedWhatsApp/,"Driver contact opens a validated prepared WhatsApp URL");
expect(deterministicDriverMessages,/amountLine[\s\S]*مبلغ الشحنة المسجل|Recorded shipment amount/,"Every driver customer message receives the recorded shipment amount");
...
expect(deterministicDriverMessages,/case\s+"driver_on_the_way"[\s\S]*case\s+"driver_request_location"[\s\S]*case\s+"driver_arrived"[\s\S]*case\s+"driver_unreachable"[\s\S]*case\s+"driver_delivered_feedback"/,"Every visible driver button has its own deterministic message body");
