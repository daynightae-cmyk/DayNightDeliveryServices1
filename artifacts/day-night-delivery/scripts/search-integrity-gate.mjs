...
const adminProfessional = read("src/components/admin/AdminOrderWorkspaceProfessional.tsx");
...
assert.match(adminCompleteRouter, /AdminOrderWorkspaceProfessional/);
assert.match(adminWorkspace, /const showLegacyBulkUi = showBulkConsole && !useProfessionalOrderRegister/);
assert.match(adminWorkspace, /searchManaged=\{showLegacyBulkUi\}/);
assert.match(adminProfessional, /searchText\(order\)\.includes\(normalizedQuery\)/);
assert.match(adminProfessional, /!searchManaged &&/);
