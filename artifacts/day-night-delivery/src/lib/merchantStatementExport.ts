...
type ColumnKey = "index" | "order" | "customer" | "phone" | "destination" | "date" | "customerTotal" | "deliveryFee" | "merchantDue" | "status" | "tracking";
type Column = { key: ColumnKey; ar: string; en: string; weight: number; ltr?: boolean; lines?: number };

const DEFAULT_LOGO_URL = "https://i.postimg.cc/XqnP282D/cropped-circle-image-(9).png";
...
const columns: Column[] = [
  { key: "index", ar: "#", en: "#", weight: 0.42, ltr: true },
  { key: "order", ar: "الطلب / الكوبون", en: "Order / Coupon", weight: 1.32, ltr: true, lines: 2 },
  { key: "customer", ar: "العميل", en: "Customer", weight: 1.05, lines: 2 },
  { key: "phone", ar: "الهاتف", en: "Phone", weight: 1.0, ltr: true },
  { key: "destination", ar: "عنوان التسليم", en: "Destination", weight: 1.9, lines: 2 },
  { key: "date", ar: "التاريخ", en: "Date", weight: 0.94, ltr: true },
  { key: "customerTotal", ar: "إجمالي العميل", en: "Customer total", weight: 1.0, ltr: true },
  { key: "deliveryFee", ar: "التوصيل", en: "Delivery", weight: 0.9, ltr: true },
  { key: "merchantDue", ar: "تسوية التاجر", en: "Merchant settlement", weight: 1.12, ltr: true, lines: 2 },
  { key: "status", ar: "الحالة", en: "Status", weight: 0.95, lines: 2 },
  { key: "tracking", ar: "الخدمات", en: "Services", weight: 1.15, lines: 2 },
];
...
export function buildMerchantStatementCsv(payload: MerchantStatementPayload) {
  ...
  downloadBlob(new Blob([csv], { type: "text/csv;charset=utf-8" }), `${safeFileName(payload)}.csv`);
}
...
function drawWrappedText(
  ...
  options: { size?: number; weight?: string; color?: string; maxLines?: number; direction?: CanvasDirection } = {},
) {
...
function drawCircularLogo(ctx: CanvasRenderingContext2D, image: HTMLImageElement | null, x: number, y: number, size: number) {
  ctx.save();
  ctx.beginPath();
...
function rowValue(row: MerchantStatementRow, key: ColumnKey, language: MerchantStatementLanguage) {
...
function drawHeader(
  ...
  const isArabic = payload.language === "ar";
  const margin = 22;
  roundedRect(ctx, margin, 20, width - margin * 2, 76, 16);
  ctx.fillStyle = "#03101f";
  ctx.fill();
  ctx.fillStyle = "#d4af37";
  ctx.fillRect(margin, 91, width - margin * 2, 5);

  const logoX = isArabic ? width - margin - 56 : margin;
  drawCircularLogo(ctx, logo, logoX, 30, 48);
  const brandX = isArabic ? logoX - 12 : logoX + 60;
  ...
  setFont(ctx, 9.5, "600", "#ffffff");
  ctx.fillText(isArabic ? "داي نايت لخدمات التوصيل والشحن" : "DAY NIGHT DELIVERY SERVICES", brandX, 66, 270);

  const metaX = isArabic ? margin + 10 : width - margin - 10;
  const metaAlign: CanvasTextAlign = isArabic ? "left" : "right";
  setFont(ctx, 8.5, "600", "#ffffff");
  ctx.textAlign = metaAlign;
  ctx.direction = isArabic ? "rtl" : "ltr";
  ctx.fillText(`${isArabic ? "تاريخ الإصدار" : "Issued"}: ${new Date().toLocaleString(isArabic ? "ar-AE" : "en-AE")}`, metaX, 45, 250);
  setFont(ctx, 9, "800", "#d4af37");
  ctx.fillText(`${isArabic ? "صفحة" : "Page"} ${page} / ${totalPages}`, metaX, 67, 160);
}

function drawMerchantCard(ctx: CanvasRenderingContext2D, payload: MerchantStatementPayload, width: number) {
  const isArabic = payload.language === "ar";
  const margin = 22;
  const y = 112;
  setFont(ctx, 18, "900", "#03101f");
  ...
  ctx.direction = isArabic ? "rtl" : "ltr";
  ctx.fillText(
    isArabic ? `كشف حساب وطلبات التاجر - ${payload.merchant.tradeName}` : `Merchant orders and account statement - ${payload.merchant.tradeName}`,
    isArabic ? width - margin : margin,
    y,
    width - margin * 2,
  );

  roundedRect(ctx, margin, 130, width - margin * 2, 54, 11);
  ...
  details.forEach(([label, value], index) => {
    ...
    const itemY = 138 + row * 21;
    setFont(ctx, 7.5, "800", "#926f00");
    ctx.textAlign = isArabic ? "right" : "left";
    ctx.direction = isArabic ? "rtl" : "ltr";
    ctx.fillText(label, isArabic ? x + itemWidth : x, itemY, itemWidth);
    setFont(ctx, 8.5, "600", "#0b172a");
    ctx.fillText(fitSingleLine(ctx, value, itemWidth - 48), isArabic ? x + itemWidth - 43 : x + 43, itemY, itemWidth - 43);
  });
}

function drawTotals(ctx: CanvasRenderingContext2D, payload: MerchantStatementPayload, width: number) {
  const isArabic = payload.language === "ar";
  const margin = 22;
  const y = 194;
  const gap = 7;
  const totals = [
    ...
    [isArabic ? "إجمالي العميل" : "Customer total", money(payload.totals.customerTotal, payload.language)],
    [isArabic ? "الرصيد النهائي" : "Final balance", merchantSettlement(payload.totals.merchantBalance, payload.language)],
  ];
  const cardWidth = (width - margin * 2 - gap * (totals.length - 1)) / totals.length;
...
function drawTable(
  ...
  rects.forEach((column) => {
    ...
    drawWrappedText(ctx, isArabic ? column.ar : column.en, column.x + column.width / 2, startY, column.width, headerHeight, "center", {
      size: 7.6,
      weight: "900",
      color: "#03101f",
      maxLines: 2,
  ...
  rows.forEach((row, rowIndex) => {
    rects.forEach((column) => {
      ...
      if (column.key === "tracking") {
        ...
        ctx.stroke();
        drawWrappedText(ctx, value, column.x + column.width / 2, y + 10, column.width - 10, rowHeight - 20, "center", {
          size: 7.6,
          weight: "900",
          color: "#0057b8",
          maxLines: 2,
        ...
        links.push({ x: column.x + 3, y: y + 7, width: column.width - 6, height: rowHeight - 14, url: row.trackingUrl });
        return;
      }

      const align: CanvasTextAlign = column.ltr ? "center" : "center";
      drawWrappedText(ctx, value, column.x + column.width / 2, y, column.width, rowHeight, align, {
        size: column.key === "destination" ? 7.7 : 8,
        weight: column.key === "merchantDue" ? "800" : "600",
        color: column.key === "merchantDue" ? (row.merchantDue < 0 ? "#b42318" : "#16794b") : "#0b172a",
...
function drawFooter(ctx: CanvasRenderingContext2D, payload: MerchantStatementPayload, width: number, height: number) {
  const isArabic = payload.language === "ar";
  const margin = 22;
  const links: PdfLink[] = [];
...
function drawPage(
  ...
  if (page === 1) {
    ...
    ctx.direction = payload.language === "ar" ? "rtl" : "ltr";
    ctx.fillText(
      payload.language === "ar" ? `تكملة كشف التاجر - ${payload.merchant.tradeName}` : `Merchant statement continued - ${payload.merchant.tradeName}`,
...
export async function buildMerchantStatementPdf(payload: MerchantStatementPayload) {
  if (typeof window === "undefined" || typeof document === "undefined") return;
  const logo = await loadLogo(payload.logoUrl || DEFAULT_LOGO_URL);
  const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "a4", compress: true });
  const width = doc.internal.pageSize.getWidth();
  const height = doc.internal.pageSize.getHeight();
  const firstPageCapacity = Math.max(3, Math.floor((height - 58 - 246 - 28) / 48));
  const laterPageCapacity = Math.max(5, Math.floor((height - 58 - 126 - 28) / 48));
  const pages: MerchantStatementRow[][] = [];
  pages.push(payload.rows.slice(0, firstPageCapacity));
  for (let index = firstPageCapacity; index < payload.rows.length; index += laterPageCapacity) {
  ...
  pages.forEach((pageRows, index) => {
    ...
    doc.addImage(rendered.canvas.toDataURL("image/jpeg", 0.94), "JPEG", 0, 0, width, height, undefined, "FAST");
