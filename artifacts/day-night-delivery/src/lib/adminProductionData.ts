...
export function productionTableForSection(id: string) {
  ...
  if (id === "import") return { table: "import_batches", dateColumn: "created_at" };
...
export async function syncAdminProductionRows(): Promise<AdminProductionSyncResult> {
  if (!supabase) return { ok: false, synced: false, message: "Supabase is not configured." };
  const { data, error } = await supabase.rpc("admin_sync_order_operation_rows");
  if (error) {
    return { ok: false, synced: false, message: safeMessage(error, "Could not sync operation rows from database.") };
  }
  return { ok: true, synced: true, details: (data || {}) as Record<string, unknown> };
}
...
export async function fetchProductionRows(sectionId: string, filters: AdminProductionFilters = {}): Promise<AdminProductionRowsResult> {
  ...
  if (!supabase) return { rows: [], source: "unavailable", table, synced: false, message: "Supabase is not configured." };

  try {
    if (sectionId === "reports") {
      const [expenses, adjustments, cod, merchant, driver, print, imports, audit] = await Promise.all([
        ...
        fetchTableRows("import_batches", "created_at", filters).catch(() => [] as FinanceRow[]),
        fetchTableRows("admin_audit_events", "created_at", filters).catch(() => [] as FinanceRow[]),
      ]);
      return {
        rows: [...expenses, ...adjustments, ...cod, ...merchant, ...driver, ...print, ...imports, ...audit],
        source: "db",
        table: "admin_production_tables",
        synced: sync.synced,
...
export async function fetchProductionFinanceSummary(): Promise<AdminProductionFinanceResult> {
  ...
  return { summary: null, source: "unavailable", message: safeMessage(rpc.error || view.error, "Finance summary is not connected to the database.") };
}
...
export function rowDebit(row: FinanceRow) {
  return rowAmount(row, ["debit", "expense_amount", "invalid_rows"]);
}

export function rowCredit(row: FinanceRow) {
  return rowAmount(row, ["credit", "amount", "cod_amount", "collected_amount", "delivery_fee", "payout_amount", "valid_rows"]);
}

export function rowBalance(row: FinanceRow) {
  return rowAmount(row, ["balance", "net_amount", "cod_amount", "collected_amount", "amount", "credit", "committed_rows"]);
}
...
export function rowReference(row: FinanceRow) {
  return String(row.tracking_number || row.reference_number || row.file_name || row.entity_id || row.order_id || row.id || "—");
}

export function rowType(row: FinanceRow) {
  return String(row.entry_type || row.adjustment_type || row.job_type || row.import_mode || row.category || row.action || row.status || "—");
}

export function rowNotes(row: FinanceRow) {
  return String(row.notes || row.reason || row.duplicate_warning || row.entity_type || row.file_name || "—");
}
...
export function summarizeRows(rows: FinanceRow[]) {
  ...
  return {
    ...
    pending: rows.filter((row) => /draft|pending|queued|preview/.test(String(row.status || ""))).length,
  };
}
