...
import { AlertTriangle, BadgePercent, Check, FileText, MessageSquare, PackageCheck, RefreshCw, Search, Truck } from "lucide-react";
...
export default function Pricing() {
  ...
  const domesticPdf = () => {
    const quote = { pickupCity: domesticPickupCity, deliveryCity: domesticDeliveryCity, service: LOCAL_SERVICE, weight: domesticWeight, pieces: 1, basePrice: domestic.total, expressCharge: 0, extraPiecesCharge: 0, total: domestic.total };
  ...
  const domesticTxt = () => exportQuoteTXT("domestic", { "Pickup Area": domesticPickupCity, "Delivery Area": domesticDeliveryCity, "Local Order": "Single order", Service: LOCAL_SERVICE, Total: formatAED(domestic.total) });
  ...
  return <div className={`space-y-8 ${isArabic ? "text-right" : "text-left"}`} dir={isArabic ? "rtl" : "ltr"}>
    <DNPageShell kicker={<><BadgePercent className="h-4 w-4" /> {tp.badge}</>} title={tp.title} subtitle={isArabic ? "أسعار واضحة للتوصيل المحلي والشحن الدولي إلى الخليج وأوروبا وأمريكا وكندا والوجهات العالمية." : "Clear prices for local UAE delivery and international shipping to GCC, Europe, America, Canada and worldwide destinations."} actions={<><Link to="/request" className="dn-btn dn-btn-primary dn-btn-md"><Truck className="h-4 w-4" />{tp.requestDelivery}</Link><a href={companyMeta.whatsappUrl} target="_blank" rel="noopener noreferrer" className="dn-btn dn-btn-whatsapp dn-btn-md"><MessageSquare className="h-4 w-4" />WhatsApp</a></>}>
