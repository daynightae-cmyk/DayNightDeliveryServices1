import { Activity, BarChart3, Database, Download, ShieldCheck, Truck } from "lucide-react";
      ...
      <section className="relative overflow-hidden rounded-[2.35rem] border border-brand-sky/20 bg-[#031226] p-5 shadow-2xl shadow-black/30 sm:p-7 lg:p-8">
        ...
        <div className="relative z-10 grid grid-cols-1 gap-6 xl:grid-cols-[1.15fr_0.85fr] xl:items-center">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-sm font-bold leading-7 text-white/58">{isArabic ? "الفلاتر، التقارير، التصدير، حالات الشحن، وسجل الحركة داخل تجربة واحدة بنفس هوية DAY NIGHT." : "Filters, reports, exports, shipment status, and order timeline in one DAY NIGHT experience."}</p>
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="col-span-2 rounded-2xl border border-brand-gold/25 bg-brand-gold/10 p-4"><div className="flex items-center gap-3"><BarChart3 className="h-5 w-5 text-brand-gold" /><div><p className="text-sm font-black text-white">{isArabic ? "ملخص تشغيلي" : "Operational summary"}</p><p className="text-xs font-bold text-white/50">{isArabic ? "متابعة وتصدير وتحديث حالات" : "Monitor, export, and update status"}</p></div></div></div>
          </div>
        </div>
      </section>
      <AdminPanelCore />
