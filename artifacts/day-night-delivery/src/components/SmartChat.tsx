...
import { Bot, Calculator, DollarSign, MapPin, MessageSquare, Minus, Navigation, Phone, Search, Send, Sparkles, Truck, X } from "lucide-react";
...
function isSpecialRoute(input: string) {
  return hasAny(input, ["special", "al ain", "dhafra", "liwa", "ruwais", "western", "العين", "الظفرة", "ليوا", "الرويس", "الغربية", "خاص", "الممتدة"]);
}
...
function operationsAnswer(input: string, isArabic: boolean) {
  if (hasAny(input, ["track", "tracking", "invoice", "phone", "coupon", "تتبع", "فاتورة", "هاتف", "رقم", "كوبون"])) {
    return isArabic
      ? `يمكنك التتبع باستخدام رقم الفاتورة أو رقم التتبع أو رقم الهاتف المسجل أو رقم الكوبون. افتح صفحة التتبع وأدخل الرقم، أو اضغط واتساب للدعم المباشر.`
      : `You can track by invoice number, tracking number, saved phone number, or coupon number. Open Tracking and enter the reference, or use WhatsApp support.`;
  }
  ...
  if (hasAny(input, ["merchant", "store", "company", "تاجر", "متجر", "شركة", "عقد"])) {
    return isArabic ? "للتجار والشركات: نوفر حساب تاجر، طلبات بالكوبون، COD، تقارير، وتتبع برقم الفاتورة أو الهاتف." : "For merchants and companies: merchant accounts, coupon orders, COD, reports, and tracking by invoice or phone.";
  }
  return null;
}
...
export default function SmartChat() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => { setShowBubble(true); sessionStorage.setItem(GREETED_KEY, "1"); }, 2200);
  ...
  useEffect(() => {
    ...
    const showContextBubble = () => {
      if (!open) setShowBubble(true);
    };

    const onFocusIn = (event: FocusEvent) => {
      const target = event.target as HTMLElement | null;
      if (target?.matches("input, textarea, select")) showContextBubble();
    };

    const onScroll = () => {
      if (window.scrollY > 420) showContextBubble();
    };
  ...
  useEffect(() => {
    if (sessionStorage.getItem(CLOSED_KEY) || sessionStorage.getItem(AUTO_OPEN_KEY)) return;
    const highIntent = ["/request", "/tracking", "/pricing"].some((route) => location.pathname.startsWith(route));
  ...
  return <>
    {showBubble && !open && <div className="dn-chat-bubble fixed max-w-[292px] pointer-events-none"><div className="relative rounded-[1.35rem] border border-brand-gold/30 bg-[#061225]/92 px-4 py-3 shadow-2xl backdrop-blur-2xl pointer-events-auto"><button onClick={() => setShowBubble(false)} className="absolute top-2 end-2 text-white/40 hover:text-white/80 transition-colors" aria-label="Dismiss bubble"><X className="w-3 h-3" /></button><p className="pe-4 text-xs font-black leading-snug text-brand-gold">{title}</p><p className="mt-1 pe-4 text-[10px] font-bold leading-5 text-white/62">{prompt}</p></div></div>}

    {open && !minimized && <div className="dn-smartchat-panel fixed overflow-hidden rounded-[1.7rem] border border-brand-gold/25 bg-[#061225]/96 shadow-2xl flex flex-col backdrop-blur-2xl" dir={isArabic ? "rtl" : "ltr"}>
      <div className="border-b border-white/10 bg-[radial-gradient(circle_at_12%_0%,rgba(24,168,232,.14),transparent_16rem),linear-gradient(135deg,rgba(10,28,58,.96),rgba(6,18,37,.94))] px-4 py-3 flex items-center justify-between gap-2"><div className="flex items-center gap-2"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-brand-gold text-brand-deep shadow-[0_0_28px_rgba(212,175,55,.25)]"><Bot className="w-5 h-5" /></div><div><p className="text-white font-black text-sm">DAY NIGHT AI</p><p className="text-white/50 text-[10px] font-bold">{title}</p></div></div><div className="flex items-center gap-1"><button onClick={() => setMinimized(true)} className="p-2 text-white/55 hover:text-brand-gold" aria-label="Minimize chat"><Minus className="w-4 h-4" /></button><button onClick={closePanel} className="p-2 text-white/55 hover:text-brand-gold" aria-label="Close chat"><X className="w-4 h-4" /></button></div></div>
      <div className="px-3 py-2 border-b border-white/10 bg-white/[0.025]"><div className="mb-2 flex items-start gap-2 rounded-2xl border border-brand-gold/18 bg-brand-gold/8 p-3"><Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-brand-gold" /><p className="text-[11px] font-bold leading-5 text-white/70">{prompt}</p></div><div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto">{quickReplies.map((item) => <button key={item} onClick={() => void sendText(item)} className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-white/5 border border-white/10 text-white/70 hover:border-brand-gold/40 hover:text-brand-gold transition-colors">{item}</button>)}</div></div>
      ...
      <div className="px-3 py-2 border-t border-white/10 grid grid-cols-4 gap-2 bg-brand-cool/50"><a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="text-[10px] font-black text-emerald-300 flex flex-col items-center gap-1"><Phone className="w-3.5 h-3.5" /> WhatsApp</a><button onClick={() => navigate("/tracking")} className="text-[10px] font-black text-white/55 hover:text-brand-gold flex flex-col items-center gap-1"><Search className="w-3.5 h-3.5" /> {isArabic ? "تتبع" : "Track"}</button><button onClick={() => navigate("/pricing")} className="text-[10px] font-black text-white/55 hover:text-brand-gold flex flex-col items-center gap-1"><Calculator className="w-3.5 h-3.5" /> {isArabic ? "سعر" : "Price"}</button><button onClick={() => navigate("/request")} className="text-[10px] font-black text-white/55 hover:text-brand-gold flex flex-col items-center gap-1"><Truck className="w-3.5 h-3.5" /> {isArabic ? "طلب" : "Order"}</button></div>
