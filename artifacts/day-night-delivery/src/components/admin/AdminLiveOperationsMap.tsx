...
import { AlertTriangle, Layers, MapPin, Navigation, Radio, Route, Satellite, Truck } from "lucide-react";
...
function MapRefresh({ points, mapMode, refreshNonce }: { points: LatLngTuple[]; mapMode: MapMode; refreshNonce: number }) {
  const map = useMap();
  useEffect(() => {
...
export default function AdminLiveOperationsMap({ isArabic, orders, selectedOrder }: AdminLiveOperationsMapProps) {
  ...
  const tileHandlers = { tileerror: () => setTileFailedByMode((current) => ({ ...current, [mapMode]: true })) };
    ...
    <div className="dn-live-map-shell dn-admin-live-ops-map relative h-[460px] w-full overflow-hidden rounded-[28px] border border-brand-gold/25 bg-[#020812]" dir={isArabic ? "rtl" : "ltr"}>
      <div className="pointer-events-none absolute left-3 right-3 top-3 z-[650] flex flex-wrap items-start justify-between gap-2">
        <div className="rounded-3xl border border-brand-gold/25 bg-[#071A33]/88 px-4 py-3 shadow-2xl backdrop-blur-xl">
          ...
          <p className="mt-1 flex items-center gap-1.5 text-[11px] font-bold text-white/65"><Satellite className="h-3.5 w-3.5 text-brand-sky" />{isArabic ? "بلاطات خرائط حقيقية + مسار طرق" : "Real tiles + road route"}</p>
          <p className="mt-1 text-[10px] font-black text-brand-gold">{isArabic ? "الوضع الحالي" : "Current mode"}: {activeModeLabel}</p>
        </div>
      ...
      <div className="absolute top-28 z-[650] flex max-w-[calc(100%-24px)] flex-wrap items-center gap-1 rounded-2xl border border-white/10 bg-[#071A33]/88 p-1 text-[10px] font-black text-white/75 backdrop-blur-xl ltr:right-3 rtl:left-3">
        <Layers className="mx-1 h-3.5 w-3.5 text-brand-gold" />
        {modeLabels.map((item) => <button key={item.mode} type="button" onClick={() => selectMode(item.mode)} className={`rounded-full px-3 py-1.5 transition ${mapMode === item.mode ? "bg-brand-gold text-brand-deep" : "hover:bg-white/10"}`}>{isArabic ? item.ar : item.en}</button>)}
      </div>

      {activeLayerFailed && <div className="pointer-events-none absolute left-1/2 top-1/2 z-[660] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-amber-300/30 bg-[#071A33]/90 px-4 py-3 text-center text-xs font-black text-amber-100 shadow-2xl backdrop-blur-xl"><AlertTriangle className="mx-auto mb-1 h-4 w-4 text-brand-gold" />{isArabic ? "تعذر تحميل هذه الطبقة مؤقتاً، جرّب وضعاً آخر." : "This layer could not be loaded temporarily; try another mode."}</div>}
      ...
      <MapContainer key={`${mapMode}-${refreshNonce}`} center={driverPos} zoom={10} style={{ height: "100%", width: "100%" }} scrollWheelZoom={false} zoomControl>
        {renderBaseLayers()}<MapRefresh points={fitPoints} mapMode={mapMode} refreshNonce={refreshNonce} />
