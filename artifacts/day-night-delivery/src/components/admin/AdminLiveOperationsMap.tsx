...
function isActiveOrder(order: AdminOrder) { return !/deliver|cancel|return|fail|complete/.test(norm(order.status)); }
function orderAmount(order: AdminOrder | null) { return Number(order?.cod_amount || order?.delivery_price || order?.price || order?.total_amount || 0); }
function label(value: unknown, fallback = "—") { return String(value || fallback); }
function orderKey(order: AdminOrder) { return String(order.id || getOrderReference(order)); }

function orderTitle(order: AdminOrder, isArabic: boolean) {
...
function includesCity(order: AdminOrder, query: string) {
...
function isStatusMatch(order: AdminOrder, filter: string) {
...
export default function AdminLiveOperationsMap({ isArabic, orders, selectedOrder }: AdminLiveOperationsMapProps) {
  ...
  const sortedOrders = useMemo(() => [...(orders || [])].sort((a, b) => new Date(String(b.created_at || b.updated_at || 0)).getTime() - new Date(String(a.created_at || a.updated_at || 0)).getTime()), [orders]);
  ...
  useEffect(() => {
    ...
    async function fetchRoute() {
      ...
      if (!activeOrder) { setRoutePoints([pickupPos, destPos]); return; }
      try {
        ...
        const data: { routes?: Array<{ distance?: number; duration?: number; geometry?: { coordinates?: Array<[number, number]> } }> } = await res.json();
        ...
        if (!cancelled && Array.isArray(coords) && coords.length > 1) {
          ...
          setRoutePoints(coords.map(([lng, lat]) => [lat, lng] as LatLngTuple));
          return;
        }
      } catch { /* safe fallback */ }
  ...
  const selectMode = (mode: MapMode) => {
    unlockAdminAudio(); playAdminAudioEvent("click"); setMapMode(mode); setRefreshNonce((value) => value + 1); setTileFailedByMode((current) => ({ ...current, [mode]: false })); writeLocal("dn_admin_map_mode", mode);
  };
  const setRegion = (region: string) => {
    unlockAdminAudio(); playAdminAudioEvent("click"); setRegionFilter(region); writeLocal("dn_admin_map_region", region); setFocusCommand({ type: "region", nonce: Date.now() });
  };
  ...
  const requestCreateShipment = () => {
    ...
    buttons.find((button) => /إضافة طلب|إضافة طلبية|New Order|Add New Order/i.test(button.textContent || ""))?.click();
  };
    ...
    <section className="dn-video-clean-live-map dn-admin-live-map-fixed w-full max-w-[920px] rounded-[28px] border border-brand-gold/20 bg-[#031226]/95 p-3 shadow-2xl shadow-black/25" dir={isArabic ? "rtl" : "ltr"} aria-label={isArabic ? "خريطة العمليات الحية" : "Live operations map"}>
      <header className="dn-map-header-clean">
        <div className="dn-map-title-row">
          <div className="min-w-0"><p><MapPin className="h-4 w-4" aria-hidden="true" />{isArabic ? "خريطة العمليات الحية" : "Live Operations Map"}</p><span dir="ltr">{String(reference).slice(0, 42)}</span></div>
          <div className="dn-map-header-actions"><AdminStateChip name="live-data" tone="success">{isArabic ? "الطبقة" : "Layer"}: {isArabic ? activeMode.ar : activeMode.en}</AdminStateChip><button type="button" className="dn-map-add-shipment" onClick={requestCreateShipment}><PackagePlus className="h-4 w-4" />{isArabic ? "إضافة طلبية" : "Add shipment"}</button></div>
        </div>
        <div className="dn-map-form-grid">
          <label><span>{isArabic ? "بحث" : "Search"}</span><div><Search className="h-3.5 w-3.5" aria-hidden="true" /><input value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder={isArabic ? "مدينة / تاجر / هاتف / رقم شحنة" : "City / merchant / phone / shipment"} /></div></label>
          <label><span>{isArabic ? "الإمارة" : "Emirate"}</span><select value={regionFilter} onChange={(event) => setRegion(event.target.value)}>{adminMapRegions.map((region) => <option key={region.id} value={region.id}>{isArabic ? region.ar : region.en}</option>)}</select></label>
          <label><span>{isArabic ? "الحالة" : "Status"}</span><select value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value); setFocusCommand({ type: "fit", nonce: Date.now() }); }}>{statusFilters.map((item) => <option key={item.id} value={item.id}>{isArabic ? item.ar : item.en}</option>)}</select></label>
          <label className="dn-map-shipment-picker"><span>{isArabic ? "اختيار الشحنة" : "Select shipment"}</span><select value={selectedOrderId} onChange={(event) => selectShipment(event.target.value)} disabled={!visibleOrders.length}>{!visibleOrders.length && <option value="">{isArabic ? "لا توجد شحنات مطابقة" : "No matching shipments"}</option>}{visibleOrders.slice(0, 120).map((order) => <option key={orderKey(order)} value={orderKey(order)}>{orderTitle(order, isArabic)}</option>)}</select></label>
        </div>
