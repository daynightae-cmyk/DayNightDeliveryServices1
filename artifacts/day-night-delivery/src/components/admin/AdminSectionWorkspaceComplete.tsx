...
const selectStatus = (value: unknown) => {
  ...
  return ORDER_STATUS_VALUES.has(normalized) ? normalized : "pending";
};
...
function FinancialCell({ order, isArabic }: { order: Order; isArabic: boolean }) {
  ...
  const rows = [
    [isArabic ? "البضاعة" : "Goods", value.goodsValue, "text-white"],
    [isArabic ? "التوصيل" : "Delivery", value.deliveryFee, "text-brand-sky"],
    [isArabic ? "الخصم" : "Discount", value.discountAmount, "text-rose-200"],
    [isArabic ? "إجمالي العميل" : "Customer total", value.customerTotal, "text-brand-gold"],
    [isArabic ? "مستحق التاجر" : "Merchant due", value.merchantDue, "text-emerald-200"],
    [isArabic ? "دخل داي نايت" : "DAY NIGHT", value.companyRevenue, "text-brand-sky"],
  ] as const;

  return (
    <div className="min-w-[220px] space-y-1 text-[10px] font-bold">
      {rows.map(([label, amount, tone]) => (
        <div className="grid grid-cols-2 gap-1" key={label}>
          <span className="rounded-lg bg-white/5 px-2 py-1 text-white/55">{label}</span>
          <strong className={`rounded-lg bg-white/5 px-2 py-1 ${tone}`} dir="ltr">
            {amount.toFixed(2)}
          </strong>
        </div>
      ))}
  ...
  async function changeOrderStatus(order: Order) {
    ...
    try {
      const ok = await updateExistingOrderStatus(
        ...
        next,
        isArabic
          ? `تحديث من لوحة الإدارة إلى ${statusText(next, true)}`
          : `Admin updated status to ${statusText(next, false)}`,
      );
      ...
      window.dispatchEvent(
        new CustomEvent("dn-admin-order-status-change", {
          detail: { orderId: order.id, status: next },
        }),
      );
      ...
      addAdminNotification({
        ...
        bodyAr:
          next === "delivered"
            ? `تم تسليم ${tracking(order)} وتأكيد التحصيل وترحيل الحساب تلقائياً.`
            : `تم تحديث ${tracking(order)} إلى ${statusText(next, true)}.`,
        bodyEn:
          next === "delivered"
            ? `${tracking(order)} delivered; collection and ledger posting were confirmed automatically.`
            : `${tracking(order)} updated to ${statusText(next, false)}.`,
      });
      setNotice(
        next === "delivered"
          ? isArabic
            ? `تم التسليم وتأكيد التحصيل وترحيل مستحق التاجر ودخل داي نايت للطلب ${tracking(order)}.`
            : `Delivery confirmed and merchant/company entries posted for ${tracking(order)}.`
          : isArabic
            ? `تم تحديث ${tracking(order)} بنجاح.`
            : `${tracking(order)} updated successfully.`,
      );
      await refresh();
    } catch (error) {
      console.error(error);
      setNotice(
        isArabic
          ? "فشل تحديث الحالة أو ترحيل الحساب. راجع Migration المالية وصلاحيات Supabase."
          : "Status or financial posting failed. Check the finance migration and Supabase permissions.",
      );
    } finally {
      setStatusBusy("");
    }
  ...
  const pdfPayload: AdminPdfPayload = {
    ...
    rows: rows.map((order) => {
      ...
      return {
        ...
        goods: money(financial.goodsValue, isArabic),
        delivery: money(financial.deliveryFee, isArabic),
        discount: money(financial.discountAmount, isArabic),
        customerTotal: money(financial.customerTotal, isArabic),
        merchantDue: money(financial.merchantDue, isArabic),
        companyRevenue: money(financial.companyRevenue, isArabic),
  ...
  return (
    <section className="dn-section-workspace" dir={isArabic ? "rtl" : "ltr"}>
      <header className="dn-section-hero">
        <div className="dn-section-hero-copy">
          ...
          <div>
            <span>
              {isArabic
                ? "إدارة الطلب والحساب · بيانات Supabase الحقيقية"
                : "Order and financial management · Live Supabase data"}
            </span>
            <h1>{title}</h1>
      ...
      <div className="dn-section-kpis">
        ...
        <article>
          <AdminIconBadge name="cod" />
          <strong>{money(customerExposure, isArabic)}</strong>
          <span>{isArabic ? "المطلوب من العملاء" : "Customer totals"}</span>
          <small>{isArabic ? "قبل/عند التسليم" : "Due/collected"}</small>
        </article>
        <article>
          <AdminIconBadge name="merchant" />
          <strong>{money(merchantExposure, isArabic)}</strong>
          <span>{isArabic ? "مستحق التجار" : "Merchant due"}</span>
          <small>{isArabic ? "بعد الخصم والتوصيل" : "After discount/fee"}</small>
        </article>
        <article>
          <AdminIconBadge name="income" />
          <strong>{money(deliveryIncome, isArabic)}</strong>
          <span>{isArabic ? "دخل داي نايت" : "DAY NIGHT revenue"}</span>
          <small>{isArabic ? "قيمة التوصيل" : "Delivery revenue"}</small>
        </article>
      </div>
      ...
      <div className="dn-section-panels">
        <article>
          ...
          <div className="dn-section-form">
            <label>
              <span>{isArabic ? "بحث شامل" : "Global search"}</span>
              <input
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder={isArabic ? "كوبون، تتبع، هاتف، تاجر، مبلغ..." : "Coupon, tracking, phone, merchant, amount..."}
              />
            </label>
          </div>
        </article>
        <article>
          <h3>
            ...
            {isArabic ? "سياسة الحساب" : "Financial policy"}
          </h3>
          <p className="dn-clean-note">
            {isArabic
              ? "الحساب يُثبت عند إدخال الطلب. عند التسليم يؤكد النظام المبلغ المحصل ويرحّل نفس القيم مرة واحدة إلى حساب التاجر وحساب داي نايت."
              : "Financials are fixed at order entry. Delivery confirms collection and posts the same values once to merchant and DAY NIGHT accounts."}
          </p>
          <button type="button" onClick={() => onNavigate?.("new_order")} className="mt-3">
            {isArabic ? "إضافة طلب مالي جديد" : "Add financial order"}
          </button>
        </article>
      </div>
      ...
      <article className="dn-section-table-card">
        <h3>
          ...
          {isArabic ? "الطلبات والتقسيم المالي" : "Orders and financial breakdown"}
        </h3>
        ...
        <div className="dn-section-table-wrap">
          <table>
            <thead>
              <tr>
                ...
                <th>{isArabic ? "التاجر والعميل" : "Merchant / customer"}</th>
            ...
            <tbody>
              {rows.map((order) => {
                ...
                const assigned =
                  extra(order).assigned_driver_id ||
                  extra(order).driver_id ||
                  order.driver_name ||
                  order.driver_code;
                ...
                return (
                  <tr key={rowKey}>
                    ...
                    <td>
                      <strong>{order.merchant_name || order.sender_name || "—"}</strong>
                    ...
                    <td>
                      <div className="flex min-w-[300px] flex-wrap gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg border border-rose-400/30 px-3 py-2 text-xs font-black text-rose-200"
                        >
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg border border-brand-gold/30 px-3 py-2 text-xs font-black text-brand-gold"
                        >
          ...
          {!rows.length && (
            <AdminEmptyState
              icon="empty-state"
              title={isArabic ? "لا توجد طلبات مطابقة" : "No matching orders"}
              message={
                isArabic
      ...
      <AdminOrderDeleteModal
        ...
        onDeleted={async () => {
          setNotice(isArabic ? "تم حذف الطلب." : "Order deleted.");
          setDeleteOrder(null);
          await refresh();
        }}
      />
      <AdminDriverAssignmentModal
        ...
        onSaved={async () => {
          setNotice(isArabic ? "تم إرسال الطلب للمندوب." : "Order dispatched.");
          setAssignOrder(null);
          await refresh();
        }}
      />
