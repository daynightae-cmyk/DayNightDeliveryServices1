...
export default function KhalifaLiveAssistant(props: Props) {
  ...
  const examples = useMemo(() => { const base = props.isArabic ? ["كم التحصيل المعلق؟", "ما أخطر مشكلة اليوم؟", "من أكثر تاجر نشاطاً؟", "ما الطلبات بدون مندوب؟", "ما صافي التشغيل؟", "ما المصروف الأعلى؟"] : ["show me unreconciled COD", "highest risk today", "top merchant", "unassigned orders", "net estimate", "top expense"]; const section = props.activeSection || ""; return /مصروف|expense/i.test(section) ? [base[5], base[4], base[1], ...base.slice(0,3)] : /تاجر|merchant/i.test(section) ? [base[2], base[0], base[4], ...base.slice(3,5)] : /مندوب|driver|pickup|إحضار/i.test(section) ? [base[3], base[0], base[1], ...base.slice(4)] : base; }, [props.isArabic, props.activeSection]);
