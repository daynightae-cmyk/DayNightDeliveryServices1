...
function answer(question: string, props: Props): string {
  ...
  const cancelled = orders.filter((o) => /cancel/.test(norm(o.status)));
  const returned = orders.filter((o) => /return/.test(norm(o.status)));
  ...
  const net = grossRevenue - expenseTotal + adjustmentNet;
...
export default function KhalifaLiveAssistant(props: Props) {
  ...
  const examples = useMemo(() => props.isArabic ? ["كم التحصيل المعلق؟", "من أكثر تاجر عليه طلبات؟", "ما صافي التشغيل؟"] : ["show me unreconciled COD", "top merchant", "net estimate"], [props.isArabic]);
  function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const trimmed = question.trim(); if (!trimmed) return; setLoading(true); window.setTimeout(() => { setHistory((items) => [{ id: `${Date.now()}`, question: trimmed, answer: answer(trimmed, props) }, ...items].slice(0, 6)); setQuestion(""); setLoading(false); }, 120); }
  return <section className="dn-khalifa-live"><form onSubmit={submit}><label>{hint}</label><textarea value={question} onChange={(event: ChangeEvent<HTMLTextAreaElement>) => setQuestion(event.target.value)} placeholder={hint} rows={3} /><div className="dn-khalifa-live-actions"><button type="submit" disabled={loading}><Send className="h-4 w-4" />{loading ? (props.isArabic ? "يفكر..." : "Thinking...") : (props.isArabic ? "إرسال" : "Send")}</button><button type="button" onClick={() => setHistory([])}><Trash2 className="h-4 w-4" />{props.isArabic ? "مسح" : "Clear"}</button></div></form><small>{note}</small><div className="dn-khalifa-examples">{examples.map((item) => <button type="button" key={item} onClick={() => setQuestion(item)}>{item}</button>)}</div>{latest && <article className="dn-khalifa-answer"><strong>{props.isArabic ? "إجابة خليفة" : "Khalifa answer"}</strong><p>{latest}</p></article>}<ul>{history.slice(0, 4).map((item) => <li key={item.id}><b>{item.question}</b><span>{item.answer}</span></li>)}</ul></section>;
}
