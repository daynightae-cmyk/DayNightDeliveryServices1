...
type Props = {
  ...
  onExportCsv: (payload: AdminPdfPayload) => void | Promise<void>;
...
export default function AdminPdfPreviewModal({ open, payload, onClose, onExportPdf, onExportCsv, onExportDoc }: Props) {
  ...
  async function run(format: "pdf" | "csv" | "doc") {
    ...
    try {
      ...
      if (format === "csv") await onExportCsv(previewPayload);
      if (format === "doc") await onExportDoc(previewPayload);
    } finally {
      setBusyFormat(null);
    }
  }
  ...
  return <div className="dn-admin-pdf-backdrop" role="dialog" aria-modal="true" dir={isArabic ? "rtl" : "ltr"}>
    <div className="dn-admin-pdf-modal">
      <header><strong>{isArabic ? "معاينة وتصدير السجل" : "Preview & export register"}</strong><button type="button" onClick={onClose}>{isArabic ? "إغلاق" : "Close"}</button></header>
      ...
      <div className="dn-admin-pdf-actions" aria-label={isArabic ? "صيغ الملف" : "File formats"}>
        <button type="button" className="dn-admin-pdf-primary" onClick={() => void run("pdf")} disabled={busyFormat !== null}><FileArchive className="h-4 w-4" />{busyFormat === "pdf" ? (isArabic ? "جاري إنشاء PDF..." : "Building PDF...") : "PDF"}</button>
        <button type="button" onClick={() => void run("csv")} disabled={busyFormat !== null}><FileSpreadsheet className="h-4 w-4" />{busyFormat === "csv" ? (isArabic ? "جاري CSV..." : "CSV...") : "CSV"}</button>
        <button type="button" onClick={() => void run("doc")} disabled={busyFormat !== null}><FileText className="h-4 w-4" />{busyFormat === "doc" ? (isArabic ? "جاري Word..." : "Word...") : "Word"}</button>
      </div>
      <small className="dn-admin-pdf-hint">{isArabic ? "PDF وCSV وWord تستخدم نفس التحديد والفلاتر واللغة الحالية. التنزيل يتطلب تحقق الإدارة الإضافي." : "PDF, CSV and Word use the same current selection, filters, and language. Download requires Admin step-up verification."}</small>
    </div>
  </div>;
}
