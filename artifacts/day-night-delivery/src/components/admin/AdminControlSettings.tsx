...
const isMapMode = (value: string | null): value is MapMode => value === "standard" || value === "satellite" || value === "terrain";
const isRegion = (value: string | null) => Boolean(value && adminMapRegions.some((region) => region.id === value));
...
function readSettings(): SettingsState {
  ...
  try {
    ...
    return {
      ...fallback,
      ...parsed,
      mapMode: isMapMode(savedMapMode) ? savedMapMode : (isMapMode(parsed.mapMode) ? parsed.mapMode : fallback.mapMode),
      region: isRegion(savedRegion) ? String(savedRegion) : (isRegion(parsed.region) ? parsed.region : fallback.region),
      khalifaAvatar: localStorage.getItem("dn_admin_khalifa_avatar") || parsed.khalifaAvatar || "",
    };
  } catch {
    return fallback;
  }
...
export default function AdminControlSettings({ isArabic, orders, merchants, financeSummary }: Props) {
  ...
  const reset = () => { const fresh: SettingsState = { theme: "night", density: "comfortable", mapMode: "standard", region: "all", glow: 72, kpiMotion: true, khalifaAvatar: "" }; setSettings(fresh); localStorage.removeItem("dn_admin_khalifa_avatar"); applySettings(fresh); };
  ...
  const subtitle = isArabic
    ? "تحكم فعلي في شكل لوحة الإدارة، صورة خليفة، الخريطة، كثافة العرض، الإشعارات، وأصوات التنبيه بدون خلط لغات."
    : "Live preferences for admin appearance, Khalifa avatar, map defaults, density, notifications, and alert sounds.";

  return <section className="dn-admin-settings-workspace">
    <header className="dn-admin-section-hero"><span>{isArabic ? "إعدادات تشغيل حقيقية" : "Live preferences"}</span><h1>{title}</h1><p>{subtitle}</p></header>
    <div className="dn-admin-section-kpis">
      ...
      <article><strong>{metrics.codRatio.toFixed(1)}%</strong><span>{isArabic ? "نسبة التحصيل عند التسليم" : "COD ratio"}</span></article>
    </div>
    <div className="dn-admin-settings-live-grid">
      <article className="dn-admin-preference-card is-wide"><div><Sun className="h-5 w-5" /><strong>{isArabic ? "الإضاءة ووضع العرض" : "Lighting & theme"}</strong></div><p>{isArabic ? "يتم تطبيق الوضع فوراً على لوحة الإدارة فقط دون التأثير على صفحات الدخول أو التتبع العامة." : "Applies immediately to admin only; /auth and /tracking are not changed."}</p><div className="dn-admin-toggle-row"><button type="button" className={settings.theme === "night" ? "is-active" : ""} onClick={() => update("theme", "night")}><Moon className="h-4 w-4" />{isArabic ? "ليلي" : "Night"}</button><button type="button" className={settings.theme === "day" ? "is-active" : ""} onClick={() => update("theme", "day")}><Sun className="h-4 w-4" />{isArabic ? "نهاري" : "Day"}</button></div><label>{isArabic ? "قوة اللمعة" : "Glow strength"}<input type="range" min="20" max="100" value={settings.glow} onChange={(e) => update("glow", Number(e.target.value))} /></label></article>
      <article className="dn-admin-preference-card"><div><ImagePlus className="h-5 w-5" /><strong>{isArabic ? "صورة خليفة" : "Khalifa avatar"}</strong></div><img className="dn-admin-avatar-preview" src={settings.khalifaAvatar || companyMeta.logoUrl} alt={isArabic ? "معاينة صورة خليفة" : "Khalifa preview"} /><input type="file" accept="image/*" onChange={(e) => uploadAvatar(e.target.files?.[0])} /></article>
      <article className="dn-admin-preference-card"><div><Map className="h-5 w-5" /><strong>{isArabic ? "الخريطة الافتراضية" : "Map defaults"}</strong></div><select value={settings.mapMode} onChange={(e) => update("mapMode", e.target.value as MapMode)}><option value="standard">{isArabic ? "خريطة عادية" : "Standard map"}</option><option value="satellite">{isArabic ? "صور فضائية" : "Satellite"}</option><option value="terrain">{isArabic ? "تضاريس" : "Terrain"}</option></select><select value={settings.region} onChange={(e) => update("region", e.target.value)}>{adminMapRegions.map((region) => <option key={region.id} value={region.id}>{isArabic ? region.ar : region.en}</option>)}</select></article>
      ...
      <article className="dn-admin-preference-card is-wide"><div><Bot className="h-5 w-5" /><strong>{isArabic ? "خليفة متصل ببيانات الإدارة" : "Live data Khalifa"}</strong></div><p>{isArabic ? `خليفة يقرأ الآن ${orders.length} طلبية، ${merchants.length} تاجر، ${metrics.unassigned} طلبية بلا مندوب، وصافي تشغيلي تقديري ${money(metrics.netEstimate, true)}.` : `Khalifa reads ${orders.length} orders, ${merchants.length} merchants, ${metrics.unassigned} unassigned orders, and estimated net ${money(metrics.netEstimate, false)}.`}</p><p>{isArabic ? "لا يتم كشف أسرار Supabase أو مفاتيح البيئة داخل هذه الصفحة." : "Supabase secrets and environment keys are not exposed here."}</p></article>
      <AdminAudioSettings isArabic={isArabic} />
      <article className="dn-admin-preference-card is-wide"><div><Bell className="h-5 w-5" /><strong>{isArabic ? "كتالوج إعدادات الإدارة" : "Admin settings catalog"}</strong></div><div className="dn-admin-settings-chip-grid">{adminSettingsCatalog.map((group) => <span key={group.id} title={(isArabic ? group.fieldsAr : group.fieldsEn).join(" • ")}>{isArabic ? group.ar : group.en}</span>)}</div></article>
    </div>
