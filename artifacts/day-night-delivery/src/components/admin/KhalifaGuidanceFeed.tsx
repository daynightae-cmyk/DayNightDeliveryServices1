...
import { AlertTriangle, CheckCircle2, ChevronLeft, ChevronRight, Clock3, Pause, Pin, Play, RotateCcw, Send, Truck, Wallet } from "lucide-react";
...
function answerFromData(question: string, isArabic: boolean, metrics: ReturnType<typeof deriveCommandMetrics>, sectionTitle?: string) {
  const q = question.toLowerCase();
  if (/cod|تحصيل|كاش/.test(q)) return isArabic ? `تحصيل COD الحالي ${amount(metrics.codTotal)}، والمعلق ${amount(metrics.codPending)}. ابدأ بمراجعة المناديب قبل إغلاق اليوم.` : `Current COD is ${amount(metrics.codTotal)} and pending COD is ${amount(metrics.codPending)}. Reconcile drivers before day close.`;
  if (/مندوب|driver|assign|توزيع/.test(q)) return isArabic ? `يوجد ${metrics.unassigned} طلب نشط بدون مندوب. افتح الخريطة وركّز على أقرب منطقة نشطة.` : `${metrics.unassigned} active orders are unassigned. Open the map and focus the nearest active region.`;
  if (/كسر|ربح|خسارة|break|net|finance/.test(q)) return isArabic ? `صافي التشغيل التقديري ${amount(metrics.netEstimate)}، ومتوسط دخل الطلب ${amount(metrics.averageOrderRevenue)}، ونسبة COD ${metrics.codRatio.toFixed(1)}%.` : `Estimated net is ${amount(metrics.netEstimate)}, average order revenue is ${amount(metrics.averageOrderRevenue)}, and COD ratio is ${metrics.codRatio.toFixed(1)}%.`;
  return isArabic ? `ملخص ${sectionTitle || "الإدارة"}: ${metrics.orders} طلب، ${metrics.active} نشط، ${metrics.delivered} مسلم، ${metrics.review} يحتاج مراجعة، وأفضل منطقة نشاط ${metrics.bestRegionId}.` : `${sectionTitle || "Admin"} summary: ${metrics.orders} orders, ${metrics.active} active, ${metrics.delivered} delivered, ${metrics.review} need review, and top active region is ${metrics.bestRegionId}.`;
}
...
export default function KhalifaGuidanceFeed({ isArabic, orders, merchants, financeSummary, sectionTitle }: Props) {
  ...
  useEffect(() => { setIndex(0); setPaused(false); setPinned(false); setAnswer(""); }, [sectionTitle, orders.length, merchants.length]);
  ...
  const ask = (event: FormEvent) => { event.preventDefault(); setAnswer(answerFromData(question, isArabic, metrics, sectionTitle)); };

  return <section className="dn-khalifa-feed dn-khalifa-rotator" aria-label={isArabic ? "تغذية خليفة" : "Khalifa Feed"}>
    <header><span>{isArabic ? "خليفة مباشر" : "Live Khalifa"}</span><strong>{sectionTitle || (isArabic ? "تنبيهات وإرشادات" : "Notifications & Guidance")}</strong></header>
    <article key={`${active.title}-${index}`} className={`dn-khalifa-current is-${active.tone}`}><div className="dn-khalifa-feed-icon"><Icon className="h-5 w-5" /></div><div className="dn-khalifa-current-body"><div className="dn-khalifa-current-top"><strong>{active.title}</strong><span>{active.severity}</span></div><p>{active.message}</p>{active.metric && <b className="dn-khalifa-current-metric">{active.metric}</b>}</div></article>
    <div className="dn-khalifa-rotator-controls"><button type="button" onClick={previous} aria-label={isArabic ? "السابق" : "Previous"}><ChevronRight className="h-4 w-4" /></button><button type="button" onClick={() => setPaused((value) => !value)}>{paused ? <Play className="h-4 w-4" /> : <Pause className="h-4 w-4" />}</button><button type="button" onClick={() => setPinned((value) => !value)} className={pinned ? "is-active" : ""}><Pin className="h-4 w-4" /></button><button type="button" onClick={next} aria-label={isArabic ? "التالي" : "Next"}><ChevronLeft className="h-4 w-4" /></button><span>{index + 1} / {visibleInsights.length}</span></div>
    <form onSubmit={ask} className="mt-3 grid grid-cols-[1fr_auto] gap-2"><input value={question} onChange={(event) => setQuestion(event.target.value)} placeholder={isArabic ? "اسأل خليفة عن COD أو المناديب أو كسر التشغيل" : "Ask Khalifa about COD, drivers, or break-even"} className="rounded-2xl border border-white/10 bg-white/[0.06] px-3 py-2 text-xs font-bold text-white outline-none" /><button type="submit" className="rounded-2xl bg-brand-gold px-3 text-brand-deep"><Send className="h-4 w-4" /></button></form>
    {answer && <div className="mt-2 rounded-2xl border border-brand-sky/20 bg-brand-sky/10 p-3 text-xs font-bold leading-6 text-white/80">{answer}</div>}
  </section>;
}
