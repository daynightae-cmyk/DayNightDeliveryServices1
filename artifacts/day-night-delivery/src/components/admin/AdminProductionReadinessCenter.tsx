...
import { AlertTriangle, BarChart3, CalendarClock, CheckCircle2, Copy, Database, FileCode2, Printer, RefreshCw } from "lucide-react";
...
export default function AdminProductionReadinessCenter({ isArabic, onNavigate }: Props) {
  ...
  return <div className="dn-admin-core-full">
    <header className="dn-section-hero"><div className="dn-section-hero-copy"><AdminIconBadge name="production-readiness" label={text(isArabic, "مركز جاهزية الإنتاج", "Production Readiness Center")} /><div><span>DAY NIGHT · {text(isArabic, "بوابة العمليات", "Operations gate")}</span><h1>{text(isArabic, "مركز جاهزية الإنتاج", "Production Readiness Center")}</h1><p>{text(isArabic, "يفحص هل لوحة الإدارة جاهزة للعمل الحقيقي على مستوى عالمي أم ما زالت تعتمد على البدائل المؤقتة.", "Checks whether admin is ready for real global operations or still depends on temporary fallbacks.")}</p></div></div><div className="dn-section-hero-actions"><button type="button" onClick={() => void refresh()} disabled={loading}><RefreshCw className="h-4 w-4" />{text(isArabic, "إعادة الفحص", "Re-run")}</button><button type="button" onClick={() => void copyReport()}><Copy className="h-4 w-4" />{copied ? text(isArabic, "تم النسخ", "Copied") : text(isArabic, "نسخ تقرير الجاهزية", "Copy report")}</button><button type="button" onClick={() => void copyFixPlan()}><FileCode2 className="h-4 w-4" />{copiedFix ? text(isArabic, "تم نسخ الخطة", "Plan copied") : text(isArabic, "نسخ خطة فتح الإنتاج", "Copy unblock plan")}</button><AdminPdfExportButton label={text(isArabic, "تصدير PDF", "Export PDF")} payload={pdfPayload} /></div></header>
    {report && <><div className="dn-admin-section-kpis"><article><AdminIconBadge name="production-readiness" className="dn-admin-kpi-icon" /><strong>{report.overallScore}%</strong><span>{text(isArabic, "الدرجة العامة", "Overall score")}</span></article><article><AdminIconBadge name="database-health" className="dn-admin-kpi-icon" /><strong>{statusText[report.status][isArabic ? "ar" : "en"]}</strong><span>{text(isArabic, "الحالة", "Status")}</span></article><article><AdminIconBadge name="priority" className="dn-admin-kpi-icon" /><strong>{riskText[report.risk][isArabic ? "ar" : "en"]}</strong><span>{text(isArabic, "الخطر", "Risk")}</span></article><article><AdminIconBadge name="error" className="dn-admin-kpi-icon" /><strong>{report.blockers}</strong><span>{text(isArabic, "موانع الإنتاج", "Blockers")}</span></article><article><AdminIconBadge name="warning" className="dn-admin-kpi-icon" /><strong>{report.warnings}</strong><span>{text(isArabic, "تحذيرات", "Warnings")}</span></article></div>
    <section className="dn-admin-section-panels"><article><h2><AdminIconBadge name="production-readiness" />{text(isArabic, "خطة تشغيل الإنتاج من غير بيانات وهمية", "Production operation plan without fake data")}</h2><p>{report.blockers ? text(isArabic, "ما زالت توجد موانع حرجة. اتبع الخطة أدناه ولا تخترع بيانات في الواجهة.", "Critical blockers still exist. Follow the plan below and do not fake UI data.") : text(isArabic, "تم تطبيق إجراءات قاعدة البيانات الأساسية. المتبقي الآن إثبات التشغيل الحقيقي: مصروف، تسوية، طباعة، وإغلاق يومي محفوظ في قاعدة البيانات.", "Core database procedures are applied. Remaining work is proving real operations: expense, adjustment, print, and a saved daily closing in the database.")}</p><ul>{requiredSqlFiles.map((file) => <li key={file}><FileCode2 className="inline h-4 w-4" /> {file}</li>)}</ul><div className="dn-section-hero-actions"><button onClick={() => void copyFixPlan()}><Copy className="h-4 w-4" />{copiedFix ? text(isArabic, "تم النسخ", "Copied") : text(isArabic, "نسخ الخطوات", "Copy steps")}</button><button onClick={() => onNavigate?.("database_health")}><Database className="h-4 w-4" />{text(isArabic, "افتح فحص قاعدة البيانات", "Open Database Health")}</button><button onClick={() => onNavigate?.("daily_closing")}><CalendarClock className="h-4 w-4" />{text(isArabic, "افتح الإغلاق اليومي", "Open Daily Closing")}</button></div></article></section>
    <div className="dn-admin-section-panels">{report.items.map((item) => <article key={item.id}><h2><AdminIconBadge name={item.status === "blocked" ? "error" : item.status === "ready" ? "success" : "warning"} />{text(isArabic, item.categoryAr, item.categoryEn)} · {item.score}%</h2><p><AdminStateChip name={item.status === "blocked" ? "error" : item.status === "ready" ? "success" : "warning"} tone={item.status === "blocked" ? "danger" : item.status === "ready" ? "success" : "warning"}>{statusText[item.status][isArabic ? "ar" : "en"]} · {riskText[item.risk][isArabic ? "ar" : "en"]}</AdminStateChip></p><p>{text(isArabic, item.messageAr, item.messageEn)}</p><strong>{text(isArabic, "الأدلة", "Evidence")}</strong><ul>{(isArabic ? item.evidenceAr : item.evidenceEn).map((evidence) => <li key={evidence}>{evidence}</li>)}</ul><strong>{text(isArabic, "الإجراء المقترح", "Recommended action")}</strong><ul>{(isArabic ? item.actionsAr : item.actionsEn).map((action) => <li key={action}>{action}</li>)}</ul></article>)}</div>
    <section className="dn-admin-section-panels"><article><h2><AlertTriangle className="h-5 w-5" />{text(isArabic, "موانع الإنتاج", "Production blockers")}</h2>{blockers.length ? <ul>{blockers.map((item) => <li key={item.id}>{text(isArabic, item.categoryAr, item.categoryEn)} — {text(isArabic, item.messageAr, item.messageEn)}</li>)}</ul> : <p><CheckCircle2 className="inline h-4 w-4" /> {text(isArabic, "لا توجد موانع حرجة حالياً.", "No critical blockers right now.")}</p>}</article><article><h2><AdminIconBadge name="click" />{text(isArabic, "الإجراءات التالية", "Next actions")}</h2><ul><li>{text(isArabic, "افتح المالية وأنشئ مصروفاً اختبارياً صغيراً ثم اعتمده أو ألغِه.", "Open Finance, create a small test expense, then approve or void it.")}</li><li>{text(isArabic, "افتح الإغلاق اليومي واحفظ أول إغلاق حقيقي داخل قاعدة البيانات.", "Open Daily Closing and save the first real closing inside the database.")}</li><li>{text(isArabic, "افتح الطباعة وأنشئ مهمة طباعة ثم علّمها مطبوعة.", "Open Print, create a print job, then mark it printed.")}</li><li>{text(isArabic, "راجع RLS Policies وأضف اختبارات E2E.", "Review RLS policies and add E2E tests.")}</li></ul><div className="dn-section-hero-actions"><button onClick={() => onNavigate?.("database_health")}><Database className="h-4 w-4" />{text(isArabic, "فتح فحص قاعدة البيانات", "Open Database Health")}</button><button onClick={() => onNavigate?.("finance_dashboard")}><BarChart3 className="h-4 w-4" />{text(isArabic, "فتح المالية", "Open Finance")}</button><button onClick={() => onNavigate?.("daily_closing")}><CalendarClock className="h-4 w-4" />{text(isArabic, "فتح الإغلاق اليومي", "Open Daily Closing")}</button><button onClick={() => onNavigate?.("print")}><Printer className="h-4 w-4" />{text(isArabic, "فتح الطباعة", "Open Print")}</button></div></article></section></>}
  </div>;
}
