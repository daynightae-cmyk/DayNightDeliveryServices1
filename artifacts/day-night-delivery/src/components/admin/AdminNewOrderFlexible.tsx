...
import { AlertTriangle, Calculator, CheckCircle2, Database, FileText, MapPin, PackagePlus, Save, ScanLine, ShieldCheck, Store } from "lucide-react";
import {
...
const inputClass = () => "w-full rounded-2xl border border-brand-sky/20 bg-brand-deep/75 px-4 py-3 text-sm font-bold text-white outline-none transition placeholder:text-white/30 focus:border-brand-gold/70 focus:ring-2 focus:ring-brand-gold/15";
const money = (value: number, isArabic: boolean) => isArabic ? `${value.toFixed(2)} درهم` : `${value.toFixed(2)} AED`;
...
export default function AdminNewOrderFlexible({
  ...
  function applyCouponReview(review: CouponPhotoReview) {
    ...
    setForm((current) => ({
      ...
      package_type: clean(imported.package_type || current.package_type || "Shipment"),
      package_description: clean(imported.package_description || imported.package_type || current.package_description),
      notes: [clean(current.notes), clean(imported.notes), `Coupon OCR ${review.result.source}; confidence ${review.confidence}%`].filter(Boolean).join(" | "),
    }));
    setSource("pending");
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await createOpsOrder({
        ...
        package_description: clean(form.package_description) || clean(form.package_type) || "Shipment",
      });
      const saved = result.row;
      const reference = clean(saved.tracking_number || saved.invoice_number || saved.id);
      let auditSuffix = "";
      if (couponReview) {
        const audit = await createAdminCouponIntakeSession({ review: couponReview, orderReference: reference, merchantId: selectedMerchant?.id || form.merchant_id });
        if (audit.warning) auditSuffix = isArabic ? ` ملاحظة الأرشفة: ${audit.warning}` : ` Archive note: ${audit.warning}`;
      }
      setSource(result.source);
      setMessage(isArabic
        ? `تم حفظ الطلب وربطه بالتاجر ${selectedMerchant?.trade_name || ""}. رقم التتبع: ${reference}. السعر: ${price.total.toFixed(2)} درهم.${auditSuffix}`
        : `Order saved and linked to ${selectedMerchant?.trade_name || "merchant"}. Tracking: ${reference}. Price: ${price.total.toFixed(2)} AED.${auditSuffix}`);
      onSaved?.(saved);
      setForm({
        ...emptyOrder,
        merchant: selectedMerchant,
      ...
      setSource("none");
      setError(isArabic ? `تعذر حفظ الطلب الحقيقي.${detail ? ` السبب: ${detail}` : ""}` : `The real order could not be saved.${detail ? ` Reason: ${detail}` : ""}`);
    } finally {
      setSaving(false);
    }
  }
    ...
    <form onSubmit={submit} className="rounded-[2rem] border border-brand-sky/20 bg-white/[0.045] p-5 shadow-2xl shadow-black/20" dir={isArabic ? "rtl" : "ltr"}>
      <header className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-white">{isArabic ? "إضافة طلب مرن — الأساسيات فقط مطلوبة" : "Flexible order — only core fields required"}</h2>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-6 text-white/55">{isArabic ? "التاجر والمستلم والهاتف والعنوان فقط إلزامية. السعر المحلي 30/50 تلقائي ويمكن استبداله بسعر يدوي." : "Only merchant, receiver, phone, and address are required. Local 30/50 pricing supports manual override."}</p>
          </div>
        </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-2 rounded-2xl border border-white/10 bg-brand-deep/55 p-2">
        <button type="button" onClick={() => setEntryMode("manual")} className={`inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-xs font-black ${entryMode === "manual" ? "bg-brand-gold text-brand-deep" : "text-white/65"}`}><FileText className="h-4 w-4" />{isArabic ? "إدخال يدوي سريع" : "Quick manual"}</button>
      ...
      {entryMode === "coupon" && (
        <div className="mb-5 space-y-3">
          ...
          {couponReview && <label className="flex gap-3 rounded-2xl border border-brand-gold/25 bg-brand-gold/5 p-4 text-xs font-bold text-white/75"><input type="checkbox" checked={reviewConfirmed} onChange={(event) => setReviewConfirmed(event.target.checked)} className="h-4 w-4 accent-amber-400" />{isArabic ? "راجعت الاسم والهاتف والعنوان والتحصيل." : "I reviewed name, phone, address, and collection."}</label>}
        </div>
      )}
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-brand-deep/35 p-4">
          <h3 className="flex items-center gap-2 text-sm font-black text-white"><Store className="h-4 w-4 text-brand-gold" />{isArabic ? "التاجر — مطلوب" : "Merchant — required"}</h3>
          ...
          {selectedMerchant && <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/5 p-3 text-xs font-bold text-emerald-100"><strong>{selectedMerchant.trade_name}</strong><span className="block opacity-70">{selectedMerchant.phone} · {selectedMerchant.pickup_address || selectedMerchant.address || selectedMerchant.emirate || "—"}</span></div>}
        </div>

        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-brand-deep/35 p-4">
          <h3 className="flex items-center gap-2 text-sm font-black text-white"><MapPin className="h-4 w-4 text-brand-gold" />{isArabic ? "المستلم والتسليم — مطلوب" : "Receiver & delivery — required"}</h3>
          <div className="grid gap-3 sm:grid-cols-2">
            <input value={form.receiver_name} onChange={(event) => setField("receiver_name", event.target.value)} placeholder={isArabic ? "اسم المستلم *" : "Receiver name *"} className={inputClass()} required />
            <input value={form.receiver_phone} onChange={(event) => setField("receiver_phone", event.target.value)} placeholder={isArabic ? "هاتف المستلم *" : "Receiver phone *"} className={inputClass()} dir="ltr" required />
          </div>
          ...
          {form.shipping_scope === "international" ? (
            <select value={form.destination_country || "SA"} onChange={(event) => setField("destination_country", event.target.value)} className={inputClass()}>{destinations.map((country) => <option key={country} value={country}>{country}</option>)}</select>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2">
              <select value={form.delivery_city} onChange={(event) => setForm((current) => ({ ...current, delivery_city: event.target.value, delivery_area: getDefaultAreaForEmirate(event.target.value) }))} className={inputClass()}>{UAE_LOCATIONS.map((location) => <option key={location.value} value={location.value}>{isArabic ? location.ar : location.en}</option>)}</select>
              <select value={form.delivery_area || ""} onChange={(event) => setField("delivery_area", event.target.value)} className={inputClass()}>{deliveryAreas.map((area) => <option key={area.value} value={area.value}>{isArabic ? area.ar : area.en}</option>)}</select>
            </div>
          )}
          <input value={form.delivery_street || form.receiver_address} onChange={(event) => setForm((current) => ({ ...current, delivery_street: event.target.value, receiver_address: event.target.value }))} placeholder={isArabic ? "عنوان التسليم التفصيلي *" : "Detailed delivery address *"} className={inputClass()} required />
        </div>
      </section>
      ...
      <section className="mt-4 grid gap-4 xl:grid-cols-[1fr_360px]">
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-brand-deep/35 p-4">
          ...
          <div className="grid grid-cols-2 gap-2 rounded-2xl border border-white/10 p-2">
            <button type="button" onClick={() => { setField("price_mode", "system"); setField("manual_delivery_price", ""); }} className={`rounded-xl px-4 py-3 text-xs font-black ${form.price_mode !== "manual" ? "bg-brand-gold text-brand-deep" : "text-white/65"}`}>{isArabic ? `سعر النظام ${price.systemTotal.toFixed(2)}` : `System ${price.systemTotal.toFixed(2)}`}</button>
            <button type="button" onClick={() => setField("price_mode", "manual")} className={`rounded-xl px-4 py-3 text-xs font-black ${form.price_mode === "manual" ? "bg-brand-gold text-brand-deep" : "text-white/65"}`}>{isArabic ? "سعر يدوي" : "Manual price"}</button>
          </div>
          ...
          {form.payment_method === "cod" && <input type="number" min={0} step="0.01" value={form.cod_amount ?? ""} onChange={(event) => setField("cod_amount", event.target.value)} placeholder={isArabic ? "مبلغ التحصيل COD *" : "COD amount *"} className={inputClass()} />}

          <details className="rounded-2xl border border-white/10 bg-black/10 p-4 text-white/70">
            <summary className="cursor-pointer text-xs font-black text-brand-gold">{isArabic ? "البيانات الاختيارية" : "Optional details"}</summary>
            <div className="mt-4 space-y-3">
              <input value={form.coupon_number || ""} onChange={(event) => setField("coupon_number", event.target.value)} placeholder={isArabic ? "رقم الكوبون — اختياري" : "Coupon — optional"} className={inputClass()} />
              <div className="grid gap-3 sm:grid-cols-2">
              ...
              <input value={form.pickup_street || ""} onChange={(event) => setField("pickup_street", event.target.value)} placeholder={isArabic ? "تفاصيل الاستلام — اختياري" : "Pickup details — optional"} className={inputClass()} />
              <input value={form.package_type} onChange={(event) => { setField("package_type", event.target.value); setField("package_description", event.target.value); }} placeholder={isArabic ? "محتوى الشحنة — اختياري" : "Package content — optional"} className={inputClass()} />
              <div className="grid gap-3 sm:grid-cols-2"><input type="number" min={1} value={form.order_count} onChange={(event) => setField("order_count", Math.max(1, Number(event.target.value) || 1))} placeholder={isArabic ? "القطع" : "Pieces"} className={inputClass()} /><input type="number" min={0.1} step="0.1" value={form.weight || 1} onChange={(event) => setField("weight", Math.max(0.1, Number(event.target.value) || 1))} placeholder={isArabic ? "الوزن" : "Weight"} className={inputClass()} /></div>
              <textarea rows={3} value={form.notes || ""} onChange={(event) => setField("notes", event.target.value)} placeholder={isArabic ? "ملاحظات — اختيارية" : "Notes — optional"} className={inputClass()} />
            </div>
          </details>
        </div>

        <aside className="space-y-3 rounded-[1.5rem] border border-brand-gold/25 bg-brand-gold/5 p-4">
          <h3 className="flex items-center gap-2 text-sm font-black text-brand-gold"><ShieldCheck className="h-4 w-4" />{isArabic ? "المراجعة النهائية" : "Final review"}</h3>
          <div className="rounded-2xl border border-white/10 bg-brand-deep/70 p-3"><span className="text-[11px] font-bold text-white/50">{form.price_mode === "manual" ? (isArabic ? "السعر اليدوي" : "Manual price") : (isArabic ? "سعر النظام" : "System price")}</span><strong className="mt-1 block text-xl text-white">{money(price.total, isArabic)}</strong>{form.price_mode === "manual" && <small className="text-white/45">{isArabic ? `مرجع النظام ${money(price.systemTotal, true)}` : `System reference ${money(price.systemTotal, false)}`}</small>}</div>
      ...
      <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-xs font-bold text-white/45">{isArabic ? "بعد الحفظ يظهر الطلب عند التاجر ويمكن تعديله أو حذفه بأمان أو إرساله للمندوب." : "After save, the order appears for the merchant and supports editing, safe deletion, and driver dispatch."}</p>
        <button type="submit" disabled={saving} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-brand-gold px-6 py-3 text-xs font-black text-brand-deep disabled:opacity-50">{saving ? <Database className="h-4 w-4 animate-pulse" /> : <Save className="h-4 w-4" />}{saving ? (isArabic ? "جارٍ الحفظ..." : "Saving...") : (isArabic ? "حفظ الطلب الحقيقي" : "Save real order")}</button>
      </div>
    </form>
  );
}
