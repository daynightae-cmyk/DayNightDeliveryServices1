...
function FinancialMetric({
  ...
  const labelClass = tone === "danger" ? "text-rose-200/90" : "text-white/50";
  const valueClass =
    tone === "danger"
      ? "text-rose-300"
      : tone === "gold"
...
export default function AdminNewOrderComplete({
  ...
  function validate() {
    ...
    if (form.price_mode === "manual") {
      numericFields.push([isArabic ? "رسوم التوصيل" : "delivery fee", form.manual_delivery_price]);
    }
      ...
      >
        <div className="rounded-[2rem] border border-brand-gold/30 bg-white/[0.045] p-5 shadow-2xl shadow-black/20">
          ...
          <label className="grid gap-2">
            ...
            >
              <option value="">
                {isArabic ? "اختر التاجر أو غرض شخصي *" : "Select merchant or personal purpose *"}
              </option>
              <option value={PERSONAL_ORDER_OPTION}>
  ...
  return (
    <form
      data-admin-new-order-form="merchant"
      autoComplete="off"
      onSubmit={submit}
    ...
    >
      <header className="mb-5 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="text-xl font-black text-white">
              {isArabic
                ? "إضافة طلب — فصل مالي كامل من لحظة الإدخال"
                : "New order — complete financial separation at entry"}
            </h2>
        ...
        <div className="mb-5 space-y-3">
          ...
          {couponReview && (
            <label className="flex gap-3 rounded-2xl border border-brand-gold/25 bg-brand-gold/5 p-4 text-xs font-bold text-white/75">
              ...
              />
              {isArabic
                ? "راجعت رقم الكوبون واسم العميل والهاتف والبيانات المستخرجة."
                : "I reviewed the coupon number, customer name, phone, and extracted data."}
            </label>
          )}
        </div>
      )}
      ...
      <section className="grid gap-4 xl:grid-cols-2">
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-brand-deep/35 p-4">
          <h3 className="flex items-center gap-2 text-sm font-black text-white">
            ...
            {isArabic ? "نوع الطلب والتاجر والكوبون" : "Order purpose, merchant, and coupon"}
          </h3>
          <select
            data-admin-order-owner-select="true"
            value={ownerSelectionValue}
          ...
          <input
            ...
            placeholder={isArabic ? "رقم الكوبون — اختياري" : "Coupon number — optional"}
            className={inputClass()}
            required
            dir="ltr"
          />
        ...
        <div className="space-y-4 rounded-[1.5rem] border border-white/10 bg-brand-deep/35 p-4">
          <h3 className="flex items-center gap-2 text-sm font-black text-white">
            ...
            {isArabic ? "بيانات العميل ومكان التسليم" : "Customer and delivery location"}
          </h3>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <input
              ...
              placeholder={isArabic ? "رقم تليفون العميل *" : "Customer phone — optional"}
              className={inputClass()}
              required
              dir="ltr"
            />
          ...
          <select
            ...
            onChange={(event) => setField("shipping_scope", event.target.value as "local" | "international")}
            className={inputClass()}
          >
          ...
          {form.shipping_scope === "international" ? (
            <label className="space-y-1">
              <span className="text-[10px] font-black text-white/50">
                {isArabic ? "دولة التسليم *" : "Destination country *"}
              </span>
              <select
          ...
          <input
            ...
            placeholder={isArabic ? "العنوان التفصيلي — اختياري" : "Detailed address — optional"}
            className={inputClass()}
          />
        </div>
      </section>
      ...
      <section
        ...
        data-final-financial-value={finalFinancialValue}
        className="mt-4 rounded-[1.7rem] border border-brand-gold/30 bg-[linear-gradient(135deg,rgba(212,175,55,0.11),rgba(11,95,255,0.08),rgba(3,18,38,0.55))] p-4 sm:p-5"
      >
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="flex items-center gap-2 text-base font-black text-white">
              <WalletCards className="h-5 w-5 text-brand-gold" />
              {isArabic ? "التقسيم المالي للطلب" : "Order financial breakdown"}
            </h3>
            <p className="mt-1 text-xs font-bold text-white/50">
              {isArabic
                ? "القيم الصفرية غير المهمة لا تظهر في الملخص، وتُثبت القيم الفعلية عند إنشاء الطلب."
                : "Unneeded zero values stay hidden; actual financial values are fixed when the order is created."}
            </p>
          </div>
          <span className="rounded-full border border-brand-gold/30 bg-brand-gold/10 px-3 py-1 text-[10px] font-black text-brand-gold">
            {isArabic ? "حساب تلقائي مباشر" : "Live automatic calculation"}
          </span>
        </div>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <label className="space-y-2">
            <span className="flex items-center gap-2 text-xs font-black text-white">
              <ReceiptText className="h-4 w-4 text-brand-sky" />
              {isArabic ? "قيمة البضاعة — يمكن أن تكون صفرًا" : "Goods value — zero allowed"}
            </span>
            <input
              type="number"
              min={0}
              step="0.01"
              data-admin-financial-input="true"
              data-admin-financial-field="goods_value"
              data-react-financial-value={String(form.goods_value ?? "")}
              autoComplete="off"
              ...
              onChange={(event) => setFinancialField("goods_value", event.currentTarget.value)}
              placeholder="100.00"
              className={inputClass()}
              required
            />
            <small className="text-[10px] font-bold text-white/40">
              {isArabic ? "ثمن منتجات التاجر" : "Merchant product value"}
            </small>
          </label>
          <label className="space-y-2">
            <span className="flex items-center gap-2 text-xs font-black text-white">
              <Calculator className="h-4 w-4 text-brand-sky" />
              {isArabic ? "قيمة التوصيل" : "Delivery fee"}
            </span>
            <div className="grid grid-cols-2 gap-2 rounded-2xl border border-white/10 p-2">
              <button
                type="button"
                onClick={() => setPriceMode("system")}
                className={`rounded-xl px-3 py-2 text-[10px] font-black ${form.price_mode !== "manual" ? "bg-brand-gold text-brand-deep" : "text-white/65"}`}
              >
                {isArabic ? `النظام ${pricing.systemTotal.toFixed(2)}` : `System ${pricing.systemTotal.toFixed(2)}`}
              </button>
              <button
                type="button"
            ...
            {form.price_mode === "manual" ? (
              <input
                ...
                step="0.01"
                data-admin-financial-input="true"
                data-admin-financial-field="manual_delivery_price"
                data-react-financial-value={String(form.manual_delivery_price ?? "")}
                autoComplete="off"
                aria-autocomplete="none"
                inputMode="decimal"
                data-form-type="other"
                ...
                onChange={(event) => setFinancialField("manual_delivery_price", event.currentTarget.value)}
                placeholder="25.00"
                className={inputClass()}
              />
            ) : (
              <div
                className="rounded-2xl border border-brand-sky/20 bg-brand-sky/5 px-4 py-3 text-lg font-black text-brand-sky"
                dir="ltr"
              >
          ...
          <label className="space-y-2">
            <span className="flex items-center gap-2 text-xs font-black text-white">
              <Landmark className="h-4 w-4 text-brand-sky" />
              {isArabic ? "الخصم — اختياري" : "Discount — optional"}
            </span>
            <input
              type="number"
              min={0}
              step="0.01"
              data-admin-financial-input="true"
              data-admin-financial-field="discount_amount"
              data-react-financial-value={String(form.discount_amount ?? "")}
              autoComplete="off"
              aria-autocomplete="none"
              inputMode="decimal"
              data-form-type="other"
              ...
              onChange={(event) => setFinancialField("discount_amount", event.currentTarget.value)}
              placeholder={isArabic ? "اتركه فارغًا بدون خصم" : "Leave blank when there is no discount"}
              className={inputClass()}
            />
            <small className="text-[10px] font-bold text-white/40">
              {isArabic ? "لا يظهر في الملخص عندما تكون قيمته صفرًا" : "Hidden from the summary when its value is zero"}
            </small>
          </label>
        </div>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <button
            type="button"
            onClick={() => setDeliveryFeeMode("customer_pays")}
            className={`rounded-2xl border p-4 text-start transition ${form.delivery_fee_mode === "customer_pays" ? "border-brand-gold/55 bg-brand-gold/15 text-brand-gold" : "border-white/10 bg-black/10 text-white/60"}`}
          >
            <strong className="block text-xs font-black">
              {isArabic ? "رسوم التوصيل تُضاف على العميل" : "Customer pays delivery fee"}
            </strong>
            <small className="mt-1 block text-[10px] font-bold opacity-70">
              {isArabic ? "الإجمالي = البضاعة + التوصيل − الخصم" : "Total = goods + delivery − discount"}
            </small>
          </button>
          <button
            type="button"
            onClick={() => setDeliveryFeeMode("deduct_from_merchant")}
            className={`rounded-2xl border p-4 text-start transition ${form.delivery_fee_mode === "deduct_from_merchant" ? "border-brand-gold/55 bg-brand-gold/15 text-brand-gold" : "border-white/10 bg-black/10 text-white/60"}`}
          >
            <strong className="block text-xs font-black">
              {isArabic ? "رسوم التوصيل على حساب التاجر" : "Charge delivery to merchant"}
            </strong>
            <small className="mt-1 block text-[10px] font-bold opacity-70">
              {isArabic
                ? "يظهر المبلغ بوضوح كمستحق على التاجر عند وجود رصيد عليه"
                : "A merchant liability is shown clearly as due from the merchant"}
            </small>
          </button>
        </div>
        ...
        {financials && settlement ? (
          <div
            className={`mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 ${financials.discountAmount > 0 ? "2xl:grid-cols-7" : "2xl:grid-cols-6"}`}
          >
            <FinancialMetric
              isArabic={isArabic}
              label={isArabic ? "قيمة البضاعة" : "Goods"}
              value={financials.goodsValue}
            />
            <FinancialMetric
              isArabic={isArabic}
              label={isArabic ? "التوصيل" : "Delivery"}
              value={financials.deliveryFee}
            />
          ...
          <div className="mt-4 rounded-2xl border border-rose-400/25 bg-rose-400/8 p-3 text-xs font-bold text-rose-100">
            {isArabic
              ? "راجع الخصم والقيم المالية لإظهار الإجمالي."
              : "Check the discount and financial values to display totals."}
          </div>
        )}
      </section>
