...
import AdminNewOrderCompleteBase from "./AdminNewOrderCompleteBase";

type AdminNewOrderCompleteProps = {
  isArabic: boolean;
  merchants: Merchant[];
...
export default function AdminNewOrderComplete(props: AdminNewOrderCompleteProps) {
  ...
  async function handleSubmitCapture(event: FormEvent<HTMLDivElement>) {
    ...
    // Leave empty-field validation to the existing form.
    if (!coupon) return;
    ...
    try {
      ...
      // The authoritative preflight passed. Submit once through the original form.
  ...
  return (
    <div onSubmitCapture={handleSubmitCapture}>
      ...
      <AdminNewOrderCompleteBase {...props} onSaved={handleSaved} />
    </div>
  );
}
