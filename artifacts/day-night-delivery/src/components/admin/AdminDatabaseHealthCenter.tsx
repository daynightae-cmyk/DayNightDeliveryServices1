...
export default function AdminDatabaseHealthCenter({ isArabic, onNavigate }: Props) {
  ...
  async function refresh() {
    ...
    try {
      ...
      const missing = next.filter((check) => check.status === "missing" || check.status === "error");
      const permission = next.filter((check) => check.status === "permission");
      if (missing.length > 0) addAdminNotification({ type: "database", sectionId: "database_health", priority: "high", dedupeKey: `db-missing:${missing.map((check) => check.id).join("|")}`, audioEvent: "database_health_warning", titleAr: "نقص في Supabase migrations", titleEn: "Supabase migrations missing", bodyAr: `يوجد ${missing.length} عنصر يحتاج migration: ${missing.map((check) => check.id).join(", ")}.`, bodyEn: `${missing.length} objects need migration: ${missing.map((check) => check.id).join(", ")}.` });
      else if (permission.length === 0 && next.length > 0) addAdminNotification({ type: "success", sectionId: "database_health", priority: "normal", dedupeKey: "db-health-ok", audioEvent: "database_health_ok", titleAr: "قاعدة البيانات جاهزة", titleEn: "Database health ready", bodyAr: "كل الفحوصات تعمل أو لا توجد بيانات فقط.", bodyEn: "All checks are working or only empty." });
    } catch (error) {
