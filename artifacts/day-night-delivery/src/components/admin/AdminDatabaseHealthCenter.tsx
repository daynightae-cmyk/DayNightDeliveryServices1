...
import { AdminIconBadge, AdminStateChip, type AdminIconName, type AdminIconTone } from "./adminIconSystem";
...
export default function AdminDatabaseHealthCenter({ isArabic, onNavigate }: Props) {
  ...
  return <section className="dn-admin-section-workspace">
    ...
    <div className="dn-section-kpis">{([{ key: "total", ar: "إجمالي الفحوصات", en: "Total checks", icon: "database-health" }, { key: "ok", ar: "جاهز", en: "Ready", icon: "success" }, { key: "missing", ar: "غير مطبق", en: "Not applied", icon: "error" }, { key: "permission", ar: "صلاحيات", en: "Permissions", icon: "warning" }, { key: "empty", ar: "جاهز بلا بيانات", en: "Ready, no rows", icon: "info" }] as const).map((item) => <article key={item.key}><AdminIconBadge name={item.icon} label={isArabic ? item.ar : item.en} className="dn-admin-kpi-icon" /><strong>{summary[item.key]}</strong><span>{isArabic ? item.ar : item.en}</span></article>)}</div>
    <article className="dn-admin-filter-table-card"><h2><AdminIconBadge name="rows" />{isArabic ? "نتائج الفحص المفصلة" : "Detailed check results"}</h2><div className="dn-admin-filter-table-wrap"><table className="dn-admin-filter-table"><thead><tr><th>{isArabic ? "العنصر" : "Object"}</th><th>{isArabic ? "القسم" : "Area"}</th><th>{isArabic ? "النوع" : "Type"}</th><th>{isArabic ? "الحالة" : "Status"}</th><th>{isArabic ? "الصفوف" : "Rows"}</th><th>{isArabic ? "المعنى" : "Meaning"}</th><th>{isArabic ? "الإجراء" : "Action"}</th></tr></thead><tbody>{checks.map((check) => <tr key={check.id}><td><b>{objectName(check, isArabic)}</b><br /><small>{check.id}</small></td><td>{areaName(check, isArabic)}</td><td>{isArabic ? kindText[check.kind].ar : kindText[check.kind].en}</td><td><AdminStateChip name={statusChip(check.status).name} tone={statusChip(check.status).tone}>{isArabic ? statusText[check.status].ar : statusText[check.status].en}</AdminStateChip></td><td>{typeof check.rowCount === "number" ? check.rowCount : "—"}</td><td>{statusMessage(check, isArabic)}</td><td>{actionText(check, isArabic)}</td></tr>)}</tbody></table></div></article>
