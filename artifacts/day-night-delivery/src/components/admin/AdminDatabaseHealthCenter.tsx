...
export default function AdminDatabaseHealthCenter({ isArabic, onNavigate }: Props) {
  ...
  return <section className="dn-admin-section-workspace">
    ...
    <article className="dn-admin-support-card"><h2>{isArabic ? "تعليمات تطبيق migration" : "Migration application instructions"}</h2><p>{isArabic ? "إذا ظهر أن finance_summary أو admin_daily_closings غير موجودة، فهذا يعني أن الكود تم دمجه لكن SQL migration لم يُطبق بعد على Supabase." : "If finance_summary or admin_daily_closings appears missing, code was merged but the SQL migration has not been applied to Supabase yet."}</p><ol><li>Open Supabase Dashboard.</li><li>Go to SQL Editor.</li><li>Apply migrations in order: 20260710072000_finance_summary_view_rpc.sql, 20260710120000_admin_daily_closing.sql, then 20260711010000_admin_production_foundation.sql.</li><li>{isArabic ? "أعد تشغيل مركز فحص قاعدة البيانات." : "Re-run Database Health Center."}</li><li>{isArabic ? "تأكد أن كل الفحوصات خضراء أو صفراء." : "Confirm all checks are green or yellow."}</li></ol><div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => onNavigate?.("settings")}><Settings className="h-4 w-4" />{isArabic ? "فتح الإعدادات" : "Open settings"}</button><button type="button" onClick={() => onNavigate?.("finance_dashboard")}><BarChart3 className="h-4 w-4" />{isArabic ? "فتح لوحة المالية" : "Open finance dashboard"}</button><span><ShieldCheck className="inline h-4 w-4" /> {isArabic ? "لا يتم عرض أسرار Supabase أو الأخطاء الخام." : "No Supabase secrets or raw errors are shown. Temporary operation is not global-ready until migration verification passes."}</span></div></article>
  </section>;
}
