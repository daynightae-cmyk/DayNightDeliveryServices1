...
import { useAdminDrivers, type AdminDriverRow } from "../../hooks/useAdminDrivers";
...
import "../../styles/dn-nexus-live-command-map.css";
...
export default function AdminNexusLiveCommandMap({ isArabic, orders }: AdminNexusLiveCommandMapProps) {
  ...
  const candidateDriver = useMemo(() => {
    if (!candidateDriverId) return null;
    return drivers.find((driver) => driver.id === candidateDriverId) || null;
  ...
  const filteredDrivers = useMemo(() => drivers.filter((driver) => {
    ...
    return matches && Boolean(driverLocationPoint(driver.location));
    ...
    features: filteredDrivers.flatMap((driver) => {
      ...
      return [{
        ...
        properties: {
          ...
          freshness: freshnessLabel(driver.location?.last_seen_at, false),
        },
      }];
    }),
  }), [filteredDrivers]);
      ...
      try {
        ...
        localMap.on("load", () => {
          ...
          localMap.addLayer({
            ...
            filter: ["!", ["has", "point_count"]],
            paint: {
              "circle-radius": ["case", ["==", ["get", "id"], selectedDriverId], 10, 8],
              "circle-color": [
                "match", ["get", "presence"],
                "online", "#31D6A3",
                "idle", "#D4AF37",
          ...
          localMap.addLayer({
            ...
            source: "dn-nexus-routes",
            filter: ["==", ["get", "routeIndex"], 0],
            paint: { "line-color": "#23A8FF", "line-width": 6, "line-opacity": 0.92 },
            ...topSlot,
          } as any);
          ...
          localMap.on("click", "dn-nexus-driver-points", (event: any) => {
            ...
            if (Array.isArray(coordinates)) localMap.easeTo({ center: coordinates, zoom: Math.max(localMap.getZoom(), 12) });
          });
          ...
          localMap.on("click", "dn-nexus-driver-clusters", async (event: any) => {
            ...
            localMap.easeTo({ center: feature.geometry.coordinates, zoom });
          });
        ...
        setMapError(isArabic ? "تعذر تشغيل خريطة Mapbox. بقية مركز العمليات ما زال متاحًا." : "Mapbox could not initialize. The rest of the operations center remains available.");
      }
    })();
  ...
  useEffect(() => {
    ...
    if (routes[0]?.geometry?.coordinates?.length) {
      ...
      if (bounds) map.fitBounds(bounds, { padding: 72, maxZoom: 14, duration: 650 });
    }
    ...
    void (async () => {
      try {
        ...
        next.sort((a, b) => {
          ...
          const presenceScore = (item: DispatchCandidate) => candidatePresence(item) === "online" ? 0 : candidatePresence(item) === "idle" ? 1 : 2;
          const presenceDelta = presenceScore(a) - presenceScore(b);
        ...
        if (!cancelled) {
          ...
          setCandidateDriverId(current || next[0]?.id || "");
        }
      } catch (cause: any) {
    ...
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc("admin_dispatch_order_runtime", {
        p_payload: {
          ...
          action,
          note: "DAY NIGHT NEXUS live operations command center",
          force: false,
        },
      });
      if (rpcError) throw rpcError;
      if (data && typeof data === "object" && "ok" in data && data.ok === false) throw new Error("dispatch_not_confirmed");
      setDispatchMessage(isArabic ? "تم حفظ الإسناد في قاعدة البيانات وتسجيله في سجل التوزيع." : "Assignment saved to the database and recorded in dispatch history.");
  ...
  const fitOperations = useCallback(() => {
    ...
    if (bounds) map.fitBounds(bounds, { padding: 70, maxZoom: 12, duration: 650 });
  ...
  const filterButtons: Array<{ id: ViewFilter; ar: string; en: string; value: number }> = [
    { id: "all", ar: "الكل", en: "All", value: filteredDrivers.length + filteredOrders.length },
    { id: "drivers", ar: "المندوبون", en: "Drivers", value: stats.total },
    { id: "pending", ar: "بانتظار الإسناد", en: "Pending", value: stats.unassigned },
    { id: "active", ar: "مهام نشطة", en: "Active", value: stats.inProgress },
    { id: "offline", ar: "غير متصل", en: "Offline", value: stats.offline },
  ];

  return (
    <section className="dn-nexus-command-map" dir={isArabic ? "rtl" : "ltr"}>
      <header className="dn-nexus-command-map__hud">
        <div className="dn-nexus-command-map__identity">
          ...
          <div>
            <b>DAY NIGHT NEXUS · LIVE OPS</b>
            <small>{isArabic ? "بيانات حقيقية فقط · GPS + Orders + Dispatch" : "Real data only · GPS + Orders + Dispatch"}</small>
          </div>
        </div>
        <div className="dn-nexus-command-map__stats">
          <button type="button" onClick={() => setViewFilter("drivers")}><Truck size={14} /><span>{isArabic ? "Online" : "Online"}</span><b>{stats.online}</b></button>
          ...
          <button type="button" onClick={() => setViewFilter("offline")}><WifiOff size={14} /><span>{isArabic ? "Offline" : "Offline"}</span><b>{stats.offline}</b></button>
        </div>
      </header>
      ...
      <div className="dn-nexus-command-map__toolbar">
        ...
        <div className="dn-nexus-command-map__map-actions">
          <button type="button" onClick={() => setStyleMode((mode) => mode === "satellite" ? "streets" : "satellite")}>
            <Layers3 size={15} /> {styleMode === "satellite" ? (isArabic ? "شوارع" : "Streets") : (isArabic ? "فضائي" : "Satellite")}
          </button>
          <button type="button" onClick={fitOperations}><Crosshair size={15} /> {isArabic ? "ملاءمة" : "Fit"}</button>
          <button type="button" onClick={() => void refresh()} disabled={loading}><RefreshCw size={15} className={loading ? "dn-nexus-spin" : ""} /> {isArabic ? "تحديث" : "Refresh"}</button>
        </div>
      </div>

      <div className="dn-nexus-command-map__body">
        <div className="dn-nexus-command-map__canvas-wrap">
          {!accessToken && (
          ...
          <div ref={mapHostRef} className="dn-nexus-command-map__canvas" aria-label={isArabic ? "خريطة NEXUS التشغيلية" : "NEXUS operations map"} />
          <div className="dn-nexus-command-map__truth-strip">
            <span className="is-live"><span /> {stats.online} {isArabic ? "GPS مباشر" : "live GPS"}</span>
            {driversWithoutGps > 0 && <span><AlertTriangle size={12} /> {driversWithoutGps} {isArabic ? "بدون GPS" : "without GPS"}</span>}
        ...
        <aside className="dn-nexus-command-map__side">
          <section className="dn-nexus-command-map__panel">
            <div className="dn-nexus-command-map__panel-title">
              <div><PackageSearch size={16} /><span><b>{isArabic ? "Pending Dispatch" : "Pending Dispatch"}</b><small>{isArabic ? "طلبات حقيقية غير مسندة" : "Real unassigned orders"}</small></span></div>
              <strong>{stats.unassigned}</strong>
            </div>
            <div className="dn-nexus-command-map__order-list">
              {liveOrders.filter(isPendingDispatch).slice(0, 8).map((order) => {
              ...
              {!liveOrders.some(isPendingDispatch) && <div className="dn-nexus-command-map__empty"><UserRoundCheck size={22} /><span>{isArabic ? "لا توجد طلبات بانتظار الإسناد." : "No orders are waiting for dispatch."}</span></div>}
            </div>
          </section>
            ...
            <section className="dn-nexus-command-map__panel dn-nexus-command-map__dispatch">
              ...
              {(routeBusy || primaryRoute || routeError) && (
                <div className="dn-nexus-command-map__route-card">
                  <div><Navigation size={15} /><b>{isArabic ? "Traffic Route" : "Traffic Route"}</b></div>
                  {routeBusy ? <span><Loader2 size={14} className="dn-nexus-spin" /> {isArabic ? "حساب المسار…" : "Calculating…"}</span> : primaryRoute ? (
                    <div className="dn-nexus-command-map__route-metrics">
                      ...
                      <span className={`traffic-${routeTraffic}`}><b>{congestionCopy(routeTraffic, isArabic)}</b><small>{routes.length > 1 ? `${routes.length - 1} ${isArabic ? "بديل" : "alt"}` : (isArabic ? "لا بدائل" : "No alternatives")}</small></span>
                    </div>
                  ) : <span className="is-error"><AlertTriangle size={14} /> {routeError}</span>}
                </div>
              )}

              <div className="dn-nexus-command-map__candidate-head">
                <span><Truck size={14} /> {isArabic ? "مرشحو الإسناد" : "Dispatch candidates"}</span>
                {candidatesBusy && <Loader2 size={14} className="dn-nexus-spin" />}
              </div>
              ...
              <div className="dn-nexus-command-map__candidates">
                {candidates.slice(0, 6).map((candidate, index) => {
                  const chosen = candidate.id === candidateDriverId;
                  const presence = candidatePresence(candidate);
                  return (
                    <button key={candidate.id} type="button" className={`${chosen ? "is-selected" : ""} presence-${presence}`} onClick={() => { setCandidateDriverId(candidate.id); setSelectedDriverId(candidate.id); }}>
              ...
              {candidateDriverId && (
                <div className="dn-nexus-command-map__assignment-review">
                  ...
                  <button type="button" onClick={() => void assignSelected()} disabled={dispatchBusy || assignedDriverId(selectedOrder) === candidateDriverId}>
                    ...
                    {assignedDriverId(selectedOrder) === candidateDriverId ? (isArabic ? "مسند حاليًا" : "Already assigned") : (isArabic ? "تأكيد الإسناد" : "Confirm assignment")}
                  </button>
                </div>
              )}
          ...
          {selectedDriver && (
            <section className="dn-nexus-command-map__panel dn-nexus-command-map__driver-focus">
              ...
              <dl>
                ...
                <div><dt>{isArabic ? "الحالة" : "Shift"}</dt><dd>{selectedDriver.shift_status || selectedDriver.status || "—"}</dd></div>
                <div><dt>{isArabic ? "مهام نشطة" : "Active jobs"}</dt><dd>{selectedDriver.active_orders}</dd></div>
                ...
                <div><dt>{isArabic ? "الدقة" : "Accuracy"}</dt><dd>{Number.isFinite(Number(selectedDriver.location?.accuracy)) ? `±${Math.round(Number(selectedDriver.location?.accuracy))}m` : "—"}</dd></div>
              </dl>
            </section>
          )}

          {(error || !accessToken) && (
            <section className="dn-nexus-command-map__truth-warning">
              <AlertTriangle size={15} />
              <span>{error || (isArabic ? "Mapbox غير مهيأ بعد؛ لن يتم استبداله ببيانات وهمية." : "Mapbox is not configured yet; no fake map data will be substituted.")}</span>
            </section>
          )}
        </aside>
      </div>
