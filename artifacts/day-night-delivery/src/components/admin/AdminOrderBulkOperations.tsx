...
function printOrders(orders: Order[], isArabic: boolean, sectionId: AdminSectionId) {
  ...
  if (!popup) return;
  const rows = orders.map((order) => `<tr><td dir="ltr">${escapeHtml(reference(order))}</td><td>${escapeHtml(order.merchant_name || order.sender_name || "—")}</td><td>${escapeHtml(order.receiver_name || order.customer_name || "—")}</td><td dir="ltr">${escapeHtml(order.receiver_phone || order.customer_phone || "—")}</td><td>${escapeHtml(localizedOrderRoute(order, isArabic ? "ar" : "en"))}</td><td dir="ltr">${escapeHtml(formatAdminMoney(order.cod_amount, isArabic))}</td><td>${escapeHtml(statusLabel(order.status, isArabic))}</td></tr>`).join("");
