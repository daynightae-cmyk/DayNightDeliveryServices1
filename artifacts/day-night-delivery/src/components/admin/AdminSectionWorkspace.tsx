...
function metricValue(key: string, rows: Order[], merchants: Merchant[], summary: FinanceSummary | null) {
  ...
  if (/cod|balance|payable|revenue|income|expense|net|value|amount|fee|earning/.test(lower)) {
    if (lower.includes("expense")) return money(summary?.total_expenses || 0);
    if (lower.includes("pending")) return money(summary?.cod_pending || cod);
    if (lower.includes("collected")) return money(summary?.cod_collected || 0);
    if (lower.includes("net")) return money(summary?.net_estimate || revenue);
    return money(revenue || cod);
  }
  return rows.length;
}
...
export default function AdminSectionWorkspace({ id, isArabic, orders, merchants, financeSummary, financeSummarySource = "derived", financeWarning, onNavigate, onRefresh }: Props) {
  ...
  const totals = {
    ...
    visible: rows.length,
    cod: money(baseRows.reduce((sum, order) => sum + Number(order.cod_amount || 0), 0)),
    income: money(baseRows.reduce((sum, order) => sum + Number(order.delivery_price || order.price || 0), 0)),
  };
  ...
  return (
    <section className="dn-section-workspace" dir={isArabic ? "rtl" : "ltr"}>
      <header className="dn-section-hero">
        ...
        <div className="dn-section-hero-actions">
          ...
          <AdminPdfExportButton payload={{
            ...
            rows: rows.map((order) => ({ tracking: tracking(order), status: statusLabel(order.status || "", isArabic), merchant: order.merchant_name || order.sender_name || "—", route: route(order), amount: money(amount(order)) })),
          }} />
        </div>
      </header>
      ...
      <div className="dn-section-kpis">
        {config.kpis.slice(0, 8).map((key) => <article key={key}><span>{kpiLabel(key, isArabic)}</span><strong>{metricValue(key, baseRows.length ? baseRows : orders, merchants, financeSummary)}</strong><small>{config.dataSource}</small></article>)}
      </div>
      ...
      <div className="dn-section-panels">
        <article>
          ...
          <div className="dn-section-form">
            <label><span>{isArabic ? "بحث" : "Search"}</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={isArabic ? "تتبع، هاتف، تاجر..." : "Tracking, phone, merchant..."} /></label>
            {[...config.filters, ...config.inputFields].slice(0, 12).map((field) => <label key={field}><span>{translatedFieldLabel(field, isArabic)}</span><input value={filters[field] || ""} onChange={(event) => setFilters((prev) => ({ ...prev, [field]: event.target.value }))} placeholder={translatedFieldLabel(field, isArabic)} /></label>)}
          </div>
        </article>
        <article>
          <h3><Sparkles />{isArabic ? "إجراءات جاهزة" : "Ready actions"}</h3>
          <div className="dn-action-grid">{config.actions.map((action) => <button key={action} type="button" onClick={() => doAction(action)}>{actionLabel(action, isArabic)}</button>)}</div>
          <p className="dn-clean-note">{isArabic ? "كل إجراء يفتح القسم المناسب أو يعرض رسالة آمنة دون أخطاء خام." : "Each action opens the right section or shows a safe message without raw errors."}</p>
        </article>
      </div>
      ...
      <article className="dn-section-table-card">
        ...
        <div className="dn-section-table-wrap">
          <table>
            <thead><tr><th>{tableColumnLabel("tracking", isArabic)}</th><th>{tableColumnLabel("status", isArabic)}</th><th>{tableColumnLabel("merchantSender", isArabic)}</th><th>{tableColumnLabel("route", isArabic)}</th><th>{tableColumnLabel("receiver", isArabic)}</th><th>{tableColumnLabel("amount", isArabic)}</th></tr></thead>
            <tbody>{rows.map((order) => <tr key={String(order.id || tracking(order))}><td>{tracking(order)}</td><td>{statusLabel(order.status || "", isArabic)}</td><td>{order.merchant_name || order.sender_name || "—"}</td><td>{route(order)}</td><td>{order.receiver_name || order.customer_name || "—"}</td><td>{money(amount(order))}</td></tr>)}</tbody>
          </table>
