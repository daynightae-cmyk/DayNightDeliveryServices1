...
type Props = { id: AdminSectionId; isArabic: boolean; orders: any[]; merchants: Merchant[]; financeSummary: FinanceSummary | null; onNavigate?: (id: AdminSectionId) => void; onRefresh?: () => Promise<void> };
const n = (v: unknown) => String(v ?? "").toLowerCase();
const money = (v: number) => `${Number(v || 0).toFixed(2)} AED`;
const amount = (o: any) => Number(o.cod_amount || o.delivery_price || o.price || o.total || o.total_amount || 0);
const income = (o: any) => Number(o.delivery_price || o.price || o.service_fee || 0);
const tracking = (o: any) => o.tracking_number || o.invoice_number || o.coupon_number || o.id || "—";
const route = (o: any) => `${o.sender_city || o.pickup_city || "—"} → ${o.receiver_city || o.delivery_city || o.destination_country || "—"}`;
function statusMatch(o: any, id: AdminSectionId) {
  const s = n(o.status); const text = n(`${o.sender_city} ${o.receiver_city} ${o.pickup_city} ${o.delivery_city} ${o.destination_country} ${o.service_type} ${o.shipping_scope} ${o.notes}`);
...
function derivedRows(id: AdminSectionId, orders: any[]) { return orders.filter((o) => statusMatch(o, id)); }
function fieldLabel(field: string, ar: boolean) { const spaced = field.replace(/([A-Z])/g, " $1"); return ar ? spaced : spaced.charAt(0).toUpperCase() + spaced.slice(1); }
function metricValue(key: string, orders: any[], merchants: Merchant[], summary: FinanceSummary | null) {
