...
export default function HomePremium({ onNavigate }: HomePremiumProps) {
  ...
  const stats = [
    { value: "24/7", label: isArabic ? "خدمة مستمرة" : "Non-stop", tone: "gold" as const },
    { value: "30 AED", label: isArabic ? "المدن الرئيسية" : "Main cities", tone: "blue" as const },
    { value: "7+", label: isArabic ? "إمارات" : "Emirates", tone: "green" as const },
    { value: "100%", label: isArabic ? "أمان" : "Safe", tone: "gold" as const },
  ];

  return (
    <div className="space-y-14 sm:space-y-20" dir={isArabic ? "rtl" : "ltr"}>
      <section className="dn-poster-hero relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-grid-pattern px-5 py-12 shadow-2xl shadow-black/25 sm:px-10 sm:py-16">
        ...
        <div className="relative z-10 grid grid-cols-1 gap-10 lg:grid-cols-[1.04fr_0.96fr] lg:items-center">
          <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className={isArabic ? "text-right" : "text-left"}>
            <DNBadge tone="gold" className="mb-5"><BadgeCheck className="h-4 w-4" /> {isArabic ? "نغطي جميع إمارات الدولة" : "UAE Certified Delivery"}</DNBadge>
            <h1 className={`max-w-4xl text-4xl font-black leading-[1.05] tracking-tight sm:text-6xl ${isLight ? "text-[#071A33]" : "text-white"}`}>{t.home.heroTitle}<span className="mt-2 block text-brand-gold">{language === "ar" ? "داخل الإمارات والعالم" : "Across UAE & Beyond"}</span></h1>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <a id="cta_whatsapp_home" href={companyMeta.whatsappUrl} target="_blank" rel="noopener noreferrer" className="dn-btn dn-btn-whatsapp dn-btn-lg"><MessageCircle className="h-4 w-4" /> {isArabic ? "تواصل واتساب" : "WhatsApp"}</a>
            </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.15 }}>
            <DNCard premium className="p-5 sm:p-6">
              ...
              <div className="mt-5 space-y-3 rounded-2xl border border-white/10 bg-brand-deep/55 p-4">
                ...
                <div className="flex items-center justify-between gap-3 text-xs font-bold text-white/60"><span>{tP?.surchargeRange || "Weight"}</span><span className="font-mono text-brand-gold" dir="ltr">{surcharge.min === 0 && surcharge.max === 0 ? (isArabic ? "0 درهم" : "0 AED") : formatAedRange(surcharge.min, surcharge.max)}</span></div>
