  ...
  return (
    <div className="space-y-12" dir={isArabic ? "rtl" : "ltr"}>
      <section className="text-center max-w-3xl mx-auto space-y-4">
        ...
        <p className="text-white/60 text-sm leading-7">{isArabic ? "اختر منطقة الاستلام ومنطقة التسليم ليظهر سعر الطلبية الواحدة فوراً. المسارات الأساسية 25 درهم للطلب، والمسارات الخاصة 50 درهم للطلب." : "Choose pickup area and delivery area to see the single-order price instantly. Standard routes are 25 AED per order; special routes are 50 AED per order."}</p>
      </section>
      ...
      <section className="grid grid-cols-1 gap-6 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 xl:grid-cols-1">
          <div className="dn-card rounded-3xl p-5"><PackageCheck className="h-8 w-8 text-brand-gold" /><p className="mt-4 text-3xl font-black text-brand-gold" dir="ltr">25 AED</p><h3 className="mt-2 font-black text-white">{isArabic ? "المسارات الأساسية" : "Standard routes"}</h3><p className="mt-2 text-xs font-bold leading-6 text-white/45">{isArabic ? "مدن ومناطق داخلية رئيسية ضمن تغطية داي نايت." : "Key cities and internal areas covered by DAY NIGHT."}</p></div>
