...
export default function RequestDelivery({ onNavigate }: RequestDeliveryProps) {
  ...
  const deliveryPricing = {
    ...
    breakdown: [isArabic ? "توصيل محلي داخل الإمارات: 30.00 AED" : "UAE local delivery: 30.00 AED"],
      ...
      <div className="rounded-3xl border border-white/10 bg-brand-cool/30 p-6 shadow-lg sm:p-8">
        ...
        {step === 3 && (
          <div className="space-y-6 text-right">
            ...
            <div className="rounded-2xl border border-brand-gold/25 bg-brand-gold/10 px-4 py-3"><p className="text-sm font-black text-brand-gold">{isArabic ? "توصيل محلي داخل الإمارات — 30 درهم ثابت" : "Local UAE delivery — fixed AED 30"}</p><p className="mt-1 text-xs font-bold text-white/55">{deliveryPricing.notes}</p></div>
