...
function commonLines(context: DeterministicDriverMessageContext, isArabic: boolean) {
  ...
  const paymentUrl = buildBankTransferUrl({
  ...
  return {
    ...
    amountLine: amount
      ...
      : "",
    paymentLine: isArabic ? `💳 طريقة الدفع: ${paymentLabel}` : `💳 Payment method: ${paymentLabel}`,
    onlineBlock:
      context.paymentMode === "online"
        ? isArabic
          ? `\n🏦 التحويل الأونلاين إلى حسابات DAY NIGHT:\n${paymentUrl}\nالبنك المحدد: ${bank.shortName}\nاكتب رقم الشحنة ${reference} في مرجع التحويل، ثم أرسل صورة الإيصال.`
          : `\n🏦 DAY NIGHT online bank transfer:\n${paymentUrl}\nSelected bank: ${bank.shortName}\nUse shipment ${reference} as the transfer reference, then send the receipt.`
        : isArabic
          ? "\n💵 يرجى تجهيز المبلغ نقدًا عند الاستلام."
          : "\n💵 Please prepare the cash amount for delivery.",
  };
}
