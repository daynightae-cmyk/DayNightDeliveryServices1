...
export const ADMIN_COMMAND_EXPANSION_ENABLED = true;
...
export const adminSettingsCatalog = [
  ...
  { id: "alerts", ar: "التنبيهات", en: "Alerts", fieldsAr: ["الطلبات المتأخرة", "بدون مندوب", "الطلبات الراجعة", "كسر التشغيل"], fieldsEn: ["Delayed orders", "Unassigned orders", "Returned orders", "Break-even"] },
] as const;
...
export function orderRegionId(order: any) {
  const text = `${order?.sender_city || ""} ${order?.pickup_city || ""} ${order?.receiver_city || ""} ${order?.delivery_city || ""} ${order?.destination_country || ""} ${order?.shipping_scope || ""} ${order?.service_type || ""}`.toLowerCase();
  return adminMapRegions.find((region) => region.id !== "all" && region.keywords.some((keyword) => text.includes(keyword.toLowerCase())))?.id || "all";
}
