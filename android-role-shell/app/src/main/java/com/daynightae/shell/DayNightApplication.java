 ...
 * legacy site styles. Blur, opacity and transformed-ancestor composition are
 * disabled because they can create a transparent login layer in Android WebView.
 */
public final class DayNightApplication extends Application {
    ...
    private void scheduleSplashWatchdog(Activity activity) {
        ...
        for (int index = 0; index < WATCHDOG_DELAYS_MS.length; index += 1) {
            ...
            webView.postDelayed(() -> stabilizeRoleSurface(webView, attempt, force), WATCHDOG_DELAYS_MS[index]);
        }
    }
    ...
    private void stabilizeRoleSurface(WebView webView, int attempt, boolean force) {
        ...
        String roleCss = driverRole
                ? "html[data-native-shell=\"driver\"] .dn-driver-login-page::before,html[data-native-shell=\"driver\"] .dn-driver-login-page::after{display:none!important;}"
                    ...
                    + "html[data-native-shell=\"driver\"] .dn-driver-auth-card *{opacity:1!important;visibility:visible!important;content-visibility:visible!important;filter:none!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;animation:none!important;mix-blend-mode:normal!important;color:#071a33!important;-webkit-text-fill-color:currentColor!important;}"
                    ...
                    + "html[data-native-shell=\"driver\"] .dn-driver-auth-card .dn-portal-auth-primary{display:flex!important;background:#0b4db2!important;color:#fff!important;-webkit-text-fill-color:#fff!important;}"
                : "html[data-native-shell=\"merchant\"] .dn-merchant-login-v3::before,html[data-native-shell=\"merchant\"] .dn-merchant-login-v3::after{display:none!important;}"
                    ...
                    + "html[data-native-shell=\"merchant\"] .dn-merchant-login-card-v3 *{opacity:1!important;visibility:visible!important;content-visibility:visible!important;filter:none!important;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;animation:none!important;mix-blend-mode:normal!important;color:#071a33!important;-webkit-text-fill-color:currentColor!important;}"
                    ...
                    + "html[data-native-shell=\"merchant\"] .dn-merchant-login-card-v3 button[type=submit]{display:flex!important;background:#0b4db2!important;color:#fff!important;-webkit-text-fill-color:#fff!important;}";
        ...
        String script = "(function(){"
                ...
                + "if(document.head){var styleTag=document.getElementById('dn-native-last-paint-style');if(!styleTag){styleTag=document.createElement('style');styleTag.id='dn-native-last-paint-style';document.head.appendChild(styleTag);}styleTag.textContent=" + quoteForJavascript(roleCss) + ";}"
                + "var boot=document.getElementById('dn-role-boot');var root=document.getElementById('root');var rendered=!!(root&&root.childElementCount>0);"
                ...
                + "if(login){"
                + "forceMany(document.documentElement,{width:'100%',minWidth:'0',minHeight:'100%',overflowX:'hidden',opacity:'1',visibility:'visible',filter:'none',transform:'none'});"
                + "forceMany(document.body,{width:'100%',minWidth:'0',minHeight:'100%',margin:'0',padding:'0',overflowX:'hidden',opacity:'1',visibility:'visible',filter:'none',transform:'none'});"
                + "forceMany(login,{position:'fixed',inset:'0',top:'0',right:'0',bottom:'0',left:'0',zIndex:'2147483000',display:'block',width:'100vw',minWidth:'0',maxWidth:'none',height:'100dvh',minHeight:'100dvh',margin:'0',padding:'10px',overflowX:'hidden',overflowY:'auto',transform:'none',translate:'none',filter:'none',opacity:'1',visibility:'visible',contentVisibility:'visible',clip:'auto',clipPath:'none',scrollBehavior:'auto',backdropFilter:'none',WebkitBackdropFilter:'none',mixBlendMode:'normal'});"
                + "if(shell){forceMany(shell,{position:'static',display:'block',width:'100%',minWidth:'0',maxWidth:'none',height:'auto',minHeight:'0',margin:'0',padding:'0',overflow:'visible',transform:'none',translate:'none',filter:'none',opacity:'1',visibility:'visible',contentVisibility:'visible',backdropFilter:'none',WebkitBackdropFilter:'none',mixBlendMode:'normal'});}"
                + "if(visual){forceStyle(visual,'display','none');}"
                + "if(card){"
                + "var ancestor=card.parentElement;while(ancestor&&ancestor!==document.documentElement){forceMany(ancestor,{opacity:'1',visibility:'visible',contentVisibility:'visible',filter:'none',backdropFilter:'none',WebkitBackdropFilter:'none',mixBlendMode:'normal'});ancestor=ancestor.parentElement;}"
                + "forceMany(card,{position:'relative',inset:'auto',top:'auto',right:'auto',bottom:'auto',left:'auto',zIndex:'2147483001',display:'flex',flexDirection:'column',justifyContent:'flex-start',alignSelf:'auto',justifySelf:'auto',width:'100%',minWidth:'0',maxWidth:'560px',height:'auto',minHeight:'0',maxHeight:'none',margin:'0 auto',padding:'22px 17px 20px',overflow:'visible',transform:'none',translate:'none',filter:'none',opacity:'1',visibility:'visible',contentVisibility:'visible',clip:'auto',clipPath:'none',borderRadius:'24px',background:'#ffffff',color:'#071a33',border:'1px solid rgba(7,26,51,.14)',boxShadow:'0 18px 54px rgba(7,26,51,.18)',backdropFilter:'none',WebkitBackdropFilter:'none',isolation:'auto',mixBlendMode:'normal'});"
                + "Array.from(card.querySelectorAll('*')).forEach(function(child){forceMany(child,{opacity:'1',visibility:'visible',contentVisibility:'visible',filter:'none',backdropFilter:'none',WebkitBackdropFilter:'none',animation:'none',mixBlendMode:'normal',color:'#071a33',WebkitTextFillColor:'currentColor'});});"
                + "}"
                + "}"
                + "var target=card||surface;var style=target?getComputedStyle(target):null;var rect=target?target.getBoundingClientRect():null;"
                + "var first=card&&card.firstElementChild?card.firstElementChild:null;var firstStyle=first?getComputedStyle(first):null;var firstRect=first?first.getBoundingClientRect():null;"
                + "var shellStyle=shell?getComputedStyle(shell):null;var shellRect=shell?shell.getBoundingClientRect():null;"
                ...
                + "var roleReady=false;if(visible){window.scrollTo(0,0);document.documentElement.scrollTop=0;if(document.body){document.body.scrollTop=0;}if(target.dataset.dnNativePaintProbe==='1'){roleReady=true;}else{target.dataset.dnNativePaintProbe='1';void target.offsetHeight;requestAnimationFrame(function(){requestAnimationFrame(function(){target.dataset.dnNativePainted='1';});});}}"
                + "var force=" + (force ? "true" : "false") + ";if(boot&&(rendered||force)){boot.classList.add('is-complete');boot.remove();}"
                + "return 'removed='+(!document.getElementById('dn-role-boot'))+',rendered='+rendered+',login='+(!!login)+',card='+(!!card)"
                + "+',textLength='+(card?String(card.innerText||'').length:-1)+',children='+(card?card.childElementCount:-1)"
                + "+',roleVisible='+visible+',roleReady='+roleReady+',rootChildren='+(root?root.childElementCount:-1)"
                + "+',background='+(style?style.backgroundColor:'missing')+',display='+(style?style.display:'missing')+',visibility='+(style?style.visibility:'missing')+',opacity='+(style?style.opacity:'missing')"
                + "+',rect='+(rect?[Math.round(rect.left),Math.round(rect.top),Math.round(rect.width),Math.round(rect.height)].join(':'):'missing')"
                + "+',first='+(firstStyle&&firstRect?[firstStyle.display,firstStyle.visibility,firstStyle.opacity,firstStyle.color,Math.round(firstRect.left),Math.round(firstRect.top),Math.round(firstRect.width),Math.round(firstRect.height)].join(':'):'missing')"
                + "+',shell='+(shellStyle&&shellRect?[shellStyle.display,shellStyle.visibility,shellStyle.opacity,Math.round(shellRect.left),Math.round(shellRect.top),Math.round(shellRect.width),Math.round(shellRect.height)].join(':'):'missing')"
        ...
        webView.evaluateJavascript(script, result -> {
            ...
            if (result != null && result.contains("roleReady=true")) webView.postInvalidateOnAnimation();
        });
    }
