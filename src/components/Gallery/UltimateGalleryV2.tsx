...
const galleryData = {
  categories: [
    ...
    {
      id: 'tracking',
      title: '🛤️ تتبع وشحن دولي',
      titleEn: 'Tracking & International',
      icon: Globe,
      images: [
    ...
    {
      id: 'premium',
      title: '🌙 لقطات احترافية',
      titleEn: 'Premium Shots',
      icon: Camera,
      images: [
    ...
    {
      id: 'branding',
      title: '🏙️ الهوية البصرية',
      titleEn: 'Branding & Landscape',
      icon: MapPin,
      images: [
...
export default function UltimateGalleryV2() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: 'Check out this amazing delivery service!',
          url: image
        });
      } catch (error) {
  ...
  }, [selectedImage, currentImageIndex, currentImages.length]);

  const totalImages = galleryData.categories.reduce((acc, cat) => acc + cat.images.length, 0);

  return (
    <div className="min-h-screen bg-gradient-to-b from-[#071A33] via-[#0A1C3A] to-[#071A33] py-12 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="text-xl text-white/60 max-w-2xl mx-auto">
            DAY NIGHT DELIVERY SERVICES — رحلة بصرية في عالم التوصيل الاحترافي
          </p>
          <div className="w-24 h-1 bg-gradient-to-r from-[#D4AF37] to-[#F5B700] mx-auto mt-4 rounded-full" />
          <p className="text-sm text-white/40 mt-2">
            {totalImages} صورة • {galleryData.categories.length} قسم
          </p>
        </motion.div>
          ...
          {galleryData.categories.map((category) => {
            ...
            return (
              <motion.button
                ...
                className={`px-5 py-3 rounded-2xl text-sm font-medium transition-all duration-300 flex items-center gap-2 ${
                  selectedCategory.id === category.id
                    ? 'bg-gradient-to-r from-[#D4AF37] to-[#F5B700] text-[#071A33] shadow-lg shadow-[#D4AF37]/20'
        ...
        <div ref={galleryRef} className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
          {currentImages.map((image, index) => (
            <motion.div
              ...
              className="group relative rounded-2xl overflow-hidden cursor-pointer bg-white/5 backdrop-blur-xl border border-white/10"
              style={{
                aspectRatio: '9/16',
            ...
            >
              ...
              {/* Overlay */}
              <div className="absolute inset-0 bg-gradient-to-t from-[#071A33]/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  <div className="flex justify-between items-center">
                    ...
                    >
                      <Heart className={`w-5 h-5 ${likedImages.has(image) ? 'fill-red-500 text-red-500' : 'text-white'}`} />
                    </button>
        ...
        >
          <div className="text-center p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10">
            <p className="text-3xl font-bold text-[#D4AF37]">{totalImages}</p>
            <p className="text-white/60 text-sm">صورة احترافية</p>
          </div>
          <div className="text-center p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10">
            <p className="text-3xl font-bold text-[#D4AF37]">{galleryData.categories.length}</p>
            <p className="text-white/60 text-sm">قسم بصري</p>
          </div>
          <div className="text-center p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10">
            <p className="text-3xl font-bold text-[#D4AF37]">{likedImages.size}</p>
            <p className="text-white/60 text-sm">إعجاب</p>
          </div>
          <div className="text-center p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10">
            <p className="text-3xl font-bold text-[#D4AF37]">24/7</p>
            <p className="text-white/60 text-sm">خدمة مستمرة</p>
          </div>
        </motion.div>

        {/* Brand Footer */}
        <div className="mt-12 text-center text-white/40 text-sm border-t border-white/5 pt-8">
          <p className="text-[#D4AF37] font-bold">DAY NIGHT DELIVERY SERVICES</p>
          <p className="text-white/30">نصل إليك في كل وقت... وفي كل مكان</p>
          <div className="flex flex-wrap justify-center gap-4 md:gap-6 mt-3">
            <span>📞 +971 56 875 7331</span>
            <span>✉️ Admin@daynightae.com</span>
            <span>🌐 daynightae.com</span>
          </div>
        </div>
      </div>
    </div>
